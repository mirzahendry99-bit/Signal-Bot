...
#   Konstanta: BTC_TREND_LOOKBACK=3, BTC_TREND_MIN_BEARISH=2
#
...
BOT_VERSION = "9.3.1"  # ← ubah satu baris ini saja saat rilis versi baru
...
MAX_SIGNALS_CYCLE  = 13          # [v9.1] dinaikkan 8→13 — sesuai MAX_OPEN_TRADES baru
...
PAIR_COOLDOWN_HOURS = 12         # 12 jam setelah closed → pair tidak bisa signal baru
...
# [v9.2.8 fix] Minimum score per strategy — SWING butuh lebih ketat.
# Data: INTRADAY WR 60.8% (OK), SWING WR 38.9% (perlu filter).
# SWING entry butuh lebih banyak konfluensi karena hold time lebih lama.
TIER_MIN_SCORE_SWING = 3.0   # SWING: min score A+ = 3.0 (vs 2.5 untuk INTRADAY)
...
# [v9.2.2 fix] RANGING market lebih noisy → butuh score lebih tinggi.
# Score 2.5 di RANGING = false signal lebih sering vs TRENDING.
# RANGING : minimum score 3.0 (tier A+ penuh)
# TRENDING: ikut TIER_MIN_SCORE["A"] = 2.0 (normal)
MIN_SCORE_RANGING = 3.0

# ── Minimum Data Sample untuk Conf ───────────────────
# [v9.2.2 fix] Jika data historis kurang dari threshold,
# signal tetap dikirim tapi position size dipotong 50%.
# Tidak di-skip total karena pair baru memang perlu warmup data.
MIN_HIST_SAMPLE_FULL = 15   # n < 15 → half size
MIN_HIST_SAMPLE_SKIP = 3    # n < 3  → skip total (tidak cukup untuk apapun)
...
BTC_TREND_LOOKBACK    = 3   # cek 3 candle 4h terakhir (~12 jam)
BTC_TREND_MIN_BEARISH = 2   # minimal 2 dari 3 candle negatif → bearish trend
...
MAX_OPEN_TRADES        = 13   # [v9.1] dinaikkan 5→13 — lebih banyak peluang signal
MAX_PER_SECTOR         = 4    # [v9.1] dinaikkan 2→4 per sektor (BTC/AI/MEME/L2)
MAX_RISK_TOTAL         = 0.10 # [v9.1] dinaikkan 5%→10% equity — accommodate 13 slots
...
SIGNAL_EXPIRE_HOURS = {
    "INTRADAY": 72,    # was 24h → 72h (3 hari)
    "SWING":    120,   # was 72h → 120h (5 hari)
    "PUMP":       4,   # unchanged — pump harus resolve cepat
    "MICROCAP":  72,   # was 48h → 72h
}
...
BASE_POSITION_USDT = 20.0    # [v9.1] $350 equity / 13 slots ≈ $27 per posisi, base $20
MAX_POSITION_USDT  = 35.0    # [v9.1] maks ~10% dari $350 equity per posisi
MIN_POSITION_USDT  = 10.0    # [v9.1] floor $10 — masih worth it di $350 equity
...
MAX_POSITION_PCT   = 0.10    # [v9.1] diturunkan 25%→10% equity per posisi — 13 slots lebih merata
...
RISK_PER_TRADE       = 0.008  # [v9.1] diturunkan 1%→0.8% — 13 slots × 0.8% = 10.4% max total risk
...
def load_winrate_table() -> dict:
    ...
    try:
        ...
        for row in rows:
            ...
            if raw_strategy in ("MICROCAP", "PUMP"):
                continue
...
def get_btc_regime(client) -> dict:
    ...
    try:
        ...
        if len(closes_4h) >= _n:
            _changes = [
                (closes_4h[-i] - closes_4h[-i-1]) / closes_4h[-i-1] * 100
                for i in range(1, BTC_TREND_LOOKBACK + 1)
            ]
            _bearish_count   = sum(1 for c in _changes if c < 0)
            btc_bearish_trend = _bearish_count >= BTC_TREND_MIN_BEARISH
        else:
            _bearish_count   = 0
...
def portfolio_allows(sig: dict, state: dict, btc: dict) -> bool:
    ...
    # ── Check 1.5: Same-pair cap ─────────────────────────────────
    # [v9.2.9 fix] Maksimal 1 open trade per pair terlepas strategy/usia.
    # Sebelumnya: BSV bisa muncul 3x (INTRADAY × 2 + SWING × 1) karena
    # dedup hanya cek pair+strategy+side dalam DEDUP_HOURS — kalau signal
    # ke-2 masuk setelah DEDUP_HOURS, lolos. Akibatnya 3 slot dari 13
    # dipakai 1 pair → kalau SL = 3 loss sekaligus.
    if pair in state.get("open_pairs", []):
        log(f"   🧠 Portfolio SKIP {pair} [{side}] — "
            f"sudah ada open trade untuk {pair} (max 1 per pair)")
        return False
...
def run():
    ...
    for t in tickers:
        ...
        try:
            ...
            # ── SWING BUY ────────────────────────────────────
            if allow_buy and not cluster_buy_blocked and not already_sent(pair, "SWING", "BUY") and not _in_cooldown:
...
def _run_tests() -> None:
    ...
    assert_eq("MAX_NOTIONAL_PCT default = 0.85", MAX_NOTIONAL_PCT, 0.85)
    ...
    new_notional = 200.0
    assert_true(
        "Notional cap menolak penambahan yang melebihi 85% equity",
    ...
    # validate_config harus return list (boleh kosong, boleh berisi)
    assert_true("validate_config returns list", isinstance(_warnings, list))
    # Konfigurasi default sekarang: MAX_OPEN_TRADES=13 × MAX_POSITION_PCT=0.10 = 1.30
    # → harus ada warning theoretical_alloc > 100%
    _has_alloc_warn = any("MAX_OPEN_TRADES × MAX_POSITION_PCT" in w for w in _warnings)
    assert_true("validate_config mendeteksi over-allocation 130%", _has_alloc_warn)
    ...
    _mock_state_full   = {"total": 13}
    _mock_state_almost = {"total": 12}
    assert_true("total=13 >= MAX_OPEN_TRADES=13 → BLOCK",
                _mock_state_full["total"] >= MAX_OPEN_TRADES)
    assert_true("total=12 < MAX_OPEN_TRADES=13 → ALLOW",
    ...
    assert_true("HYPE score=2.5 di RANGING → SKIP",
                _hype_regime == "RANGING" and _hype_score < MIN_SCORE_RANGING)
    # Score 3.0 di RANGING masih lolos
    assert_true("Score=3.0 di RANGING → LOLOS",
                not (3.0 < MIN_SCORE_RANGING))
    # Score 2.5 di TRENDING tetap lolos (tidak kena filter ini)
    assert_true("Score=2.5 di TRENDING → LOLOS (filter hanya untuk RANGING)",
                not ("TRENDING" == "RANGING" and 2.5 < MIN_SCORE_RANGING))
    ...
    assert_true("Screenshot score masih tier A+", _screenshot_score >= TIER_MIN_SCORE["A+"])

    # Expire hours
    print("\n── [v9.2.3] Expire hours ──")
    assert_eq("INTRADAY expire = 72h", SIGNAL_EXPIRE_HOURS["INTRADAY"], 72)
    assert_eq("SWING expire = 120h",   SIGNAL_EXPIRE_HOURS["SWING"],    120)
    assert_eq("PUMP expire = 4h",      SIGNAL_EXPIRE_HOURS["PUMP"],       4)
    assert_eq("MICROCAP expire = 72h", SIGNAL_EXPIRE_HOURS["MICROCAP"],  72)
    ...
    # 2 dari 3 candle negatif → bearish trend
    _changes_bear = [-0.3, -0.5, +0.1]   # 2 negatif dari 3
    _bear = sum(1 for c in _changes_bear if c < 0)
    assert_true("2/3 candle negatif → bearish trend", _bear >= BTC_TREND_MIN_BEARISH)

    # 1 dari 3 candle negatif → TIDAK bearish
    _changes_ok = [-0.1, +0.3, +0.2]   # 1 negatif dari 3
    _ok = sum(1 for c in _changes_ok if c < 0)
    assert_true("1/3 candle negatif → bukan bearish trend", _ok < BTC_TREND_MIN_BEARISH)

    # Semua positif → tidak bearish
    _changes_bull = [+0.2, +0.4, +0.1]
    ...
    assert_eq("BTC_TREND_LOOKBACK = 3",    BTC_TREND_LOOKBACK,    3)
    assert_eq("BTC_TREND_MIN_BEARISH = 2", BTC_TREND_MIN_BEARISH, 2)
    ...
    assert_eq("TIER_MIN_SCORE_SWING = 3.0", TIER_MIN_SCORE_SWING, 3.0)
    # Score 2.5 lolos INTRADAY tapi tidak lolos SWING
    _score_25 = 2.5
    assert_true("Score 2.5 lolos INTRADAY (>= 2.5)", _score_25 >= TIER_MIN_SCORE["A+"])
    assert_true("Score 2.5 gagal SWING (< 3.0)", _score_25 < TIER_MIN_SCORE_SWING)
    # Score 3.0 lolos keduanya
    assert_true("Score 3.0 lolos SWING", 3.0 >= TIER_MIN_SCORE_SWING)
    ...
    assert_eq("PAIR_COOLDOWN_HOURS = 12", PAIR_COOLDOWN_HOURS, 12)
