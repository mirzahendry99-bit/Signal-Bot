...
print("🚀 SIGNAL BOT — DECISION ENGINE v8")
...
def flush_signal_queue(perf: dict, regime: dict = None):
    ...
    for sig in sorted_q:
        ...
        # Engine 8: Trade Narrative (dari build_trade_narrative, bukan narrative_str duplikat)
...
def check_scalping(client, pair, price, fg, ob_ratio, funding,
    ...
    for side in ["BUY", "SELL"]:
        ...
        # Recalculate TP/SL dari final_entry
        if side == "BUY":
            tp1_f = final_entry * 1.05
            tp2_f = final_entry * 1.10
            sl_f  = max(final_entry - atr*2.0, final_entry*0.96)
        else:
            tp1_f = final_entry * 0.95
            tp2_f = final_entry * 0.90
            sl_f  = min(final_entry + atr*2.0, final_entry*1.04)
        queue_signal(
...
def check_moonshot(client, pair, price, change_24h, trending, liq, regime):
    ...
    if vol_ratio > 4.0 and 25 < rsi < 70 and change_24h > 3.0:
        ...
        prec_moon   = {"entry_quality": "READY", "detail": [], "pullback": False, "rejection": False, "candle_conf": False}
        entry_info  = resolve_entry(price, "BUY", atr, {}, liq_map, structure, prec_moon)
        final_entry = entry_info["entry_price"]
        ...
        queue_signal(
            ...
            extra=extra, structure=structure, liq_map=liq_map,
            precision=prec_moon, ob={}, entry_info=entry_info,
        )
...
def run():
    ...
    client = get_client()
    print(f"=== SIGNAL SCAN | {SIGNAL_MODE} | v7 Decision Engine ===")
    ...
    tg(
        f"🔍 <b>Scan Selesai — v7 Decision Engine</b>\n"
        f"━━━━━━━━━━━━━━━━━━\n"
