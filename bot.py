"""
╔══════════════════════════════════════════════════════════════════╗
║           SIGNAL BOT — CLEAN v7.20                              ║
║                                                                  ║
║  Perbaikan v7.20 (Quant Optimization Layer):                     ║
║                                                                  ║
║  [v7.20 #A] Risk-based exposure cap (bukan locked capital)       ║
...
CORR_SIZE_PENALTY    = 0.50   # max_corr × 0.5 → size discount. corr=0.9 → size × 55%
...
def save_signal(pair: str, strategy: str, side: str, entry: float,
    ...
    try:
        supabase.table("signals_v2").insert({
            ...
            "position_size": round(position_size, 4) if position_size else None,  # [v7.18 #C]
        }).execute()
    except Exception as e:
...
def get_portfolio_state() -> dict:
    ...
    try:
        ...
        return {"total": 0, "buy": 0, "sell": 0, "locked_usdt": 0.0, "total_risk_usdt": 0.0, "portfolio_heat_pct": 0.0, "open_pairs": []}
...
def run():
    ...
    for sig in signals:
        ...
        sig["position_size"] = calc_position_size(
            ...
            current_equity=available_equity,  # [v7.19 #D] available, bukan total equity   # [v7.16 #D] equity aktif, bukan statis
        )
