"""
╔══════════════════════════════════════════════════════════════════╗
║           SIGNAL BOT — CLEAN v7.23                              ║
║                                                                  ║
║  Perbaikan v7.23 (Invalid Pair Fast-Skip):                      ║
║                                                                  ║
║  [v7.23] gate_call_with_retry: skip retry jika 400              ║
║          INVALID_CURRENCY_PAIR — pair delisted dari Gate.io.    ║
║          Sebelumnya: 3x retry × ~2s = ~6 detik terbuang/pair.  ║
║          Sekarang: return None langsung + masuk _invalid_pairs  ║
║          blacklist (in-memory per cycle) → pair sama tidak      ║
║          dicoba lagi di fungsi manapun dalam 1 run.             ║
║                                                                  ║
...
MAX_OPEN_TRADES        = 8    # total signal aktif (INTRADAY+SWING) — hard cap
MAX_SAME_SIDE_TRADES   = 5    # max BUY atau SELL aktif sekaligus
MAX_BTC_CORR_TRADES    = 4    # max BUY aktif saat BTC correlation tinggi
...
# ── Anti-Expiry System Config ── [v7.24] ─────────────────────────────────────
# Level 1: Warning dikirim saat X jam sebelum expired
EXPIRY_WARN_HOURS   = 2.0
# Level 2: Velocity filter — skip signal jika estimasi jam ke TP1 > threshold ini
# (sebagai kelipatan dari SIGNAL_EXPIRE_HOURS, misal 0.8 = 80% dari expire window)
VELOCITY_THRESHOLD  = 0.8
# Level 3: Extend expiry jika harga sudah bergerak minimal X% menuju TP1
EXPIRY_EXTEND_PROGRESS = 0.30   # 30% of way to TP1
EXPIRY_EXTEND_MULTIPLIER = 1.5  # extend expire window jadi 1.5x
...
# [v7.23] Runtime blacklist pair yang sudah confirmed delisted/invalid dari Gate.io.
...
def gate_call_with_retry(fn, *args, retries: int = 3, base_delay: float = 1.0, **kwargs):
    ...
    [v7.23]   Skip retry langsung jika 400 INVALID_CURRENCY_PAIR — pair delisted,
              retry tidak akan pernah berhasil. Hemat ~6 detik per pair invalid.
    """
    # [v7.23] Cek blacklist dulu — pair sudah diketahui invalid, skip langsung
    ...
    for attempt in range(retries):
        try:
            ...
            # [v7.23] 400 INVALID_CURRENCY_PAIR → pair delisted, jangan retry sama sekali
...
def check_intraday(client, pair: str, price: float, ob_ratio: float,
    ...
    # ── [v7.24 L2] Velocity filter — estimasi jam ke TP1 ────────────────────
    # Skip signal jika momentum terlalu lemah untuk mencapai TP1 dalam expire window.
    # Gunakan avg price change per jam dari 5 candle terakhir.
    try:
        _n_candles   = min(5, len(closes) - 1)
        _price_delta = closes[-1] - closes[-1 - _n_candles]
        _rate_pct_hr = (_price_delta / closes[-1 - _n_candles]) * 100 / _n_candles
        _dist_tp1_pct = abs(tp1 - entry) / entry * 100

        # Arah momentum harus sesuai side
        _momentum_ok = (_rate_pct_hr > 0) if side == "BUY" else (_rate_pct_hr < 0)
        if not _momentum_ok:
            _est_hours = float("inf")
        else:
            _rate_abs = abs(_rate_pct_hr)
            _est_hours = _dist_tp1_pct / _rate_abs if _rate_abs > 0 else float("inf")

        _expire_h = SIGNAL_EXPIRE_HOURS.get("INTRADAY", 24)
        if _est_hours > _expire_h * VELOCITY_THRESHOLD:
            log(f"   🐢 [L2] Velocity skip {pair} {side}: "
                f"est {_est_hours:.1f}h > {_expire_h * VELOCITY_THRESHOLD:.1f}h threshold")
            return None
    except Exception:
        pass   # velocity calc gagal → lolos filter, tidak blocking
...
def evaluate_open_trades(client) -> dict:
    ...
    for row in rows:
        ...
        # ── [v7.24 L3] Dynamic expiry extension ────────────────────
        # Jika harga sudah bergerak minimal EXPIRY_EXTEND_PROGRESS menuju TP1,
        # perpanjang expire window agar bot tidak berhenti monitor terlalu cepat.
        try:
            _tickers_l3 = gate_call_with_retry(client.list_tickers, currency_pair=pair)
            _cur_l3 = float(_tickers_l3[0].last or 0) if _tickers_l3 else 0.0
            if _cur_l3 > 0 and entry > 0 and tp1 > 0:
                _dist_total = abs(tp1 - entry)
                _dist_done  = (_cur_l3 - entry) if side == "BUY" else (entry - _cur_l3)
                _progress   = _dist_done / _dist_total if _dist_total > 0 else 0.0
                if _progress >= EXPIRY_EXTEND_PROGRESS:
                    _new_expire = expire_hours * EXPIRY_EXTEND_MULTIPLIER
                    if age_hours <= _new_expire:
                        log(f"   ⏳ [L3] Expiry extended: {pair} progress={_progress:.0%} "
                            f"→ window {expire_hours:.0f}h → {_new_expire:.0f}h")
                        expire_hours = _new_expire   # override untuk cek di bawah
        except Exception as _l3e:
            pass  # gagal fetch → pakai expire_hours asli, tidak blocking
        ...
        # ── [v7.24 L1] Near-expiry warning ──────────────────────────
        # Kirim Telegram alert saat 2 jam sebelum expired, satu kali saja.
        elif expire_hours - age_hours <= EXPIRY_WARN_HOURS:
            _warned = row.get("expiry_warned")
            if not _warned:
                try:
                    _tickers_w = gate_call_with_retry(client.list_tickers, currency_pair=pair)
                    _cur_w = float(_tickers_w[0].last or 0) if _tickers_w else 0.0
                    _sisa_jam = expire_hours - age_hours
                    if _cur_w > 0 and entry > 0:
                        _pct_w = (_cur_w - entry) / entry * 100
                        if side == "SELL": _pct_w = -_pct_w
                        _pct_str_w = f"{_pct_w:+.2f}%"
                    else:
                        _pct_str_w = "N/A"
                    pair_disp_w = pair.replace("_USDT", "/USDT")
                    tg(f"⚠️ <b>Hampir Expired — {pair_disp_w}</b>\n"
                       f"━━━━━━━━━━━━━━━━━━\n"
                       f"Strategy : {strategy} {side}\n"
                       f"Sisa     : <b>{_sisa_jam:.1f} jam</b>\n"
                       f"Posisi   : <b>{_pct_str_w}</b> dari entry\n"
                       f"━━━━━━━━━━━━━━━━━━\n"
                       f"<i>TP1 belum tercapai. Pertimbangkan close manual.</i>")
                    log(f"   ⚠️ [L1] Near-expiry warning terkirim: {pair} sisa {_sisa_jam:.1f}j")
                    # Mark warned — graceful jika kolom belum ada di schema
                    try:
                        supabase.table("signals_v2").update({"expiry_warned": True}) \
                            .eq("id", trade_id).execute()
                    except Exception:
                        pass  # kolom belum ada — warning tetap terkirim, flag tidak tersimpan
                except Exception as _we:
                    log(f"   ⚠️ Near-expiry warning gagal [{pair}]: {_we}", "warn")
        ...
        try:
            ...
            if result == "TP1_PARTIAL":
                ...
                # [v7.24] Flag tp1_notified agar notifikasi tidak terkirim ulang
                try:
                    supabase.table("signals_v2").update({"tp1_notified": True}) \
                        .eq("id", trade_id).execute()
                except Exception as _tne:
                    log(f"   ⚠️ tp1_notified flag gagal [{pair}]: {_tne}", "warn")
...
def run():
    ...
    log(f"\n{'='*60}")
    log(f"🚀 SIGNAL BOT v7.23 — {datetime.now(WIB).strftime('%Y-%m-%d %H:%M WIB')} [FULL SCAN]")
    log(f"{'='*60}")
    ...
    if not signals and micro_sent == 0:
        tg(f"🔍 <b>Scan Selesai — SIGNAL BOT v7.23</b>\n"
           f"━━━━━━━━━━━━━━━━━━\n"
