...
print("🚀 SIGNAL BOT — FINAL EDITION v3")
...
# AGGRESSIVE = lebih banyak sinyal (rekomen saat awal)
# CONSERVATIVE = akurasi lebih tinggi (setelah ada win rate data)
SIGNAL_MODE  = "AGGRESSIVE"
MIN_STRENGTH = {"CONSERVATIVE": 4, "AGGRESSIVE": 3}[SIGNAL_MODE]

MIN_RR = {
    "CONSERVATIVE": {"SCALPING": 1.8, "INTRADAY": 2.0, "SWING": 2.5, "MOONSHOT": 2.0},
    "AGGRESSIVE":   {"SCALPING": 1.2, "INTRADAY": 1.5, "SWING": 2.0, "MOONSHOT": 1.5},
}[SIGNAL_MODE]

# ── Weighted Scoring ──────────────────────────
# Sinyal high-value (smart money) dapat bobot lebih tinggi
W = {
    "divergence":  3,   # RSI divergence — reversal terkuat
    "liq_cluster": 2,   # Liquidation cluster — market magnet
    "oi_signal":   2,   # Open Interest — futures positioning
    "ichimoku":    2,   # Ichimoku cloud — structural trend
    "vwap":        2,   # VWAP — institutional fair value
    "bb_extreme":  2,   # BB extreme — oversold/overbought
    "funding":     2,   # Funding rate — crowd positioning
    "poc":         2,   # Volume POC — akumulasi/distribusi
    "rsi_extreme": 1,
    "stoch_rsi":   1,
    "ema_cross":   1,
    "ob_ratio":    1,
...
# Threshold disesuaikan dengan realitas: Coinglass sering NEUTRAL
# Skenario tanpa Coinglass maks score = ~11, threshold harus realistis
MIN_WSCORE = {
...
BTC_1H_DROP_BLOCK = -3.0  # block BUY alt jika BTC turun >3% dalam 1h
BTC_1H_PUMP_BLOCK =  3.0  # block SELL alt jika BTC naik >3% dalam 1h

# ── Pairs dengan futures contract di Gate.io ──
FUTURES_PAIRS = {
    "BTC_USDT","ETH_USDT","SOL_USDT","BNB_USDT","XRP_USDT",
    "DOGE_USDT","ADA_USDT","AVAX_USDT","DOT_USDT",
    "LINK_USDT","UNI_USDT","ATOM_USDT","LTC_USDT","NEAR_USDT",
    "ARB_USDT","OP_USDT","APT_USDT","SUI_USDT","TRX_USDT",
}

# Order book hanya untuk top liquid pairs — hemat API calls
TOP_PAIRS_OB = {
    "BTC_USDT","ETH_USDT","SOL_USDT","BNB_USDT","XRP_USDT",
...
# ═══════════════════════════════════════════════
#  EXTERNAL DATA — LAYER 1: FEAR & GREED
# ═══════════════════════════════════════════════
def get_fear_greed():
    try:
...
# ═══════════════════════════════════════════════
#  EXTERNAL DATA — LAYER 2: COINGECKO
# ═══════════════════════════════════════════════
...
# ═══════════════════════════════════════════════
#  EXTERNAL DATA — LAYER 3: COINGLASS
# ═══════════════════════════════════════════════
def get_liquidation_data(symbol):
    ...
    try:
        ...
        if liq_short > liq_long * 1.5:   bias = "BUY"
        elif liq_long > liq_short * 1.5: bias = "SELL"
        else:                             bias = "NEUTRAL"
        return {
...
# ═══════════════════════════════════════════════
#  EXTERNAL DATA — LAYER 4: GATE.IO FUTURES
# ═══════════════════════════════════════════════
...
def get_market_regime(client) -> dict:
    """
    Baca konteks market sebelum scan pair.
    Output: regime (RISK_ON/NEUTRAL/RISK_OFF), block_buy, block_sell
    """
    default = {
    ...
    try:
        ...
        if btc_trend == "BULL" and not block_buy:
            regime = "RISK_ON"
            reason = reason or "BTC > EMA200"
        elif btc_trend == "BEAR":
            regime    = "RISK_OFF"
            block_buy = True
            reason    = reason or "BTC bear market"
        else:
            regime = "NEUTRAL"
            reason = reason or "BTC sideways"

        print(f"📡 Regime: {regime} | BTC: {btc_trend} | 1h: {btc_1h_chg:+.1f}% | Dom: {btc_dom:.1f}%")
        ...
        return {
            ...
            "block_buy":  block_buy, "block_sell": block_sell,
            "reason":     reason,
        }
    except Exception as e:
...
def get_candles(client, pair, interval, limit):
    """Gate.io: [0]=ts [1]=vol [2]=close [3]=high [4]=low [5]=open"""
    try:
...
def calc_rsi_divergence(closes, highs, lows, period=14, lookback=20):
    """Deteksi divergence berbasis swing point — bukan fixed candle."""
...
def calc_volume_profile(closes, volumes, bins=10):
    """Point of Control (POC) — harga dengan volume terbesar."""
...
def update_expired_signals(client):
    """
    Auto-update result sinyal yang sudah expired.
    TP / SL / EXPIRED berdasarkan harga saat ini vs entry.
    """
    try:
        now = datetime.now(WIB)
        ...
        for sig in res.data:
            ...
            if side == "BUY":
                result = "TP" if current >= tp1 else ("SL" if current <= sl else "EXPIRED")
            else:
                result = "TP" if current <= tp1 else ("SL" if current >= sl else "EXPIRED")

            supabase.table("signals").update({"result": result}) \
...
def send_signal(pair, signal_type, side, entry, tp1, tp2, sl,
    ...
    if rr < min_rr:
        print(f"  ⏭️ {pair} [{signal_type}] skip R/R — {rr} < {min_rr}")
        return

    win_rate = get_win_rate(signal_type)
    wr_str   = f"{win_rate}%" if win_rate else "N/A (akumulasi data)"
    ...
    save_signal(pair, signal_type, side, entry, tp1, tp2, sl,
                strength, timeframe, valid_until)
    print(f"  ✅ {signal_type} {side} → {pair} | R/R:1:{rr} | Strength:{strength}")
...
def volatility_ok(atr: float, price: float, mode: str) -> tuple:
    ...
    if atr_pct < mn: return False, f"Volatilitas rendah ({atr_pct:.2f}%)"
    if atr_pct > mx: return False, f"Volatilitas terlalu tinggi ({atr_pct:.2f}%)"
    return True, ""
...
def check_scalping(client, pair, price, fg, ob_ratio, funding,
    ...
    min_ws     = MIN_WSCORE["SCALPING"]

    vol_ok, _ = volatility_ok(atr, price, "SCALPING")
    if not vol_ok: return

    # ── BUY ──────────────────────────────────
    # Di Extreme Fear: relax EMA condition karena harga sedang ditekan
    ema_ok_buy = (ema9 > ema21) or (fg < 20 and rsi < 28)
    if rsi < 35 and ema_ok_buy and vol_ratio > 1.5:
        ...
        print(f"  📊 {pair} [SCALP BUY] wscore={score}/{min_ws} rsi={rsi:.1f}")
        if score < min_ws: return

        tp1 = price * 1.05
        tp2 = min(price * 1.10, resistance * 0.99)
        ...
        if fg < 20:                      extra += f"\n😱 <i>Extreme Fear {fg} — zona akumulasi</i>"

        send_signal(pair,"SCALPING","BUY",price,tp1,tp2,sl,
                    stars_val,"5m",15,"Gate.io · F&G · OB · StochRSI · VWAP",extra)

    # ── SELL ─────────────────────────────────
    elif rsi > 68 and ema9 < ema21 and vol_ratio > 1.5:
        if regime.get("block_sell"): return
        ...
        score     = wscore(conditions)
        stars_val = min(5, max(1, score // 2))
        print(f"  📊 {pair} [SCALP SELL] wscore={score}/{min_ws} rsi={rsi:.1f}")
        if score < min_ws: return

        tp1 = price * 0.95
        tp2 = max(price * 0.90, support * 1.01)
...
def check_intraday(client, pair, price, fg, ob_ratio, funding,
    ...
    min_ws      = MIN_WSCORE["INTRADAY"]

    vol_ok, _ = volatility_ok(atr, price, "INTRADAY")
    if not vol_ok: return

    # ── BUY ──────────────────────────────────
    # Extreme Fear: longgarkan RSI threshold — F&G 13 = zona akumulasi kuat
    rsi_buy_thr = 45 if fg < 20 else 42
    if rsi < rsi_buy_thr and macd > msig and price <= bb_mid:
        if regime.get("block_buy"): return
        ...
        print(f"  📊 {pair} [INTRADAY BUY] wscore={score}/{min_ws} rsi={rsi:.1f}")
        if score < min_ws: return

        tp1 = min(price * 1.07, resistance * 0.99)
        tp2 = min(price * 1.15, resistance * 1.05)
        ...
        if ichi.get("above_cloud"):      extra += "\n☁️ <i>Ichimoku: Di atas cloud</i>"
        if liq.get("liq_bias") == "BUY": extra += f"\n💥 <i>Short liq ${liq['liq_short_above']}M</i>"

        send_signal(pair,"INTRADAY","BUY",price,tp1,tp2,sl,
                    stars_val,"1h",120,
                    "Gate.io · F&G · CoinGecko · StochRSI · VWAP · Ichimoku · Liq · OI",extra)

    # ── SELL ─────────────────────────────────
    # Extreme Fear: RSI 55+ sudah cukup untuk SELL (dead cat bounce)
    rsi_sell_thr = 55 if fg < 20 else 60
    if rsi > rsi_sell_thr and macd < msig and price >= bb_mid:
        if regime.get("block_sell"): return
        ...
        conditions = [
            ...
            (fg > 60 or fg < 20,             "fg_extreme"),   # fear ekstrem = confirm dead cat
        ...
        score     = wscore(conditions)
        stars_val = min(5, max(1, score // 3))
        print(f"  📊 {pair} [INTRADAY SELL] wscore={score}/{min_ws} rsi={rsi:.1f}")
        if score < min_ws: return

        tp1 = max(price * 0.93, support * 1.01)
        tp2 = max(price * 0.87, support * 0.95)
        sl  = min(price + atr * 2.5, price * 1.06)

        extra = f"📡 <i>Regime: {regime['regime']} | BTC 1h: {regime['btc_1h_chg']:+.1f}%</i>"
        if divergence == "BEARISH":       extra += "\n🔀 <i>Bearish RSI Divergence!</i>"
        if ichi.get("below_cloud"):       extra += "\n☁️ <i>Ichimoku: Di bawah cloud</i>"
        if liq.get("liq_bias") == "SELL": extra += f"\n💥 <i>Long liq ${liq['liq_long_below']}M</i>"
...
def check_swing(client, pair, price, fg, ob_ratio, funding,
    ...
    if rsi < 50 and rsi > 20 and macd > msig:
        if regime.get("regime") == "RISK_OFF" and pair not in ("BTC_USDT","ETH_USDT"): return
        if regime.get("block_buy"): return
        ...
        print(f"  📊 {pair} [SWING BUY] wscore={score}/{min_ws} rsi={rsi:.1f} regime={regime['regime']}")
        if score < min_ws: return

        tp1 = min(price * 1.12, resistance * 0.98)
        tp2 = min(price * 1.25, resistance * 1.10)
        ...
        send_signal(pair,"SWING","BUY",price,tp1,tp2,sl,
                    stars_val,"4h",720,
                    "Gate.io · F&G · CoinGecko · EMA50/200 · VWAP · Ichimoku · Liq · OI",extra)
    ...
    # ── SELL ─────────────────────────────────
        ...
        score     = wscore(conditions)
        stars_val = min(5, max(1, score // 4))
        print(f"  📊 {pair} [SWING SELL] wscore={score}/{min_ws} rsi={rsi:.1f}")
        if score < min_ws: return

        tp1 = max(price * 0.88, support * 1.02)
        tp2 = min(tp1 * 0.95, max(price * 0.78, support * 0.90))
...
def check_moonshot(client, pair, price, change_24h, trending, liq, regime):
    ...
    if vol_ratio > 4.0 and 25 < rsi < 70 and change_24h > 3.0:
        ...
        print(f"  📊 {pair} [MOONSHOT] wscore={score}/{min_ws} vol={vol_ratio:.1f}x")
        if score < min_ws: return
        ...
        if price > resistance: extra += f"🚀 <i>Breakout Resistance!</i>\n"
...
# ═══════════════════════════════════════════════
#  MARKET SUMMARY (1x/hari jam 08:00 WIB)
# ═══════════════════════════════════════════════
...
def send_market_summary(fg, fg_label, market_data, trending, regime):
...
def run():
    client = get_client()
    print(f"=== SIGNAL SCAN | Mode: {SIGNAL_MODE} ===")

    # Auto-update hasil sinyal expired
    print("📝 Updating expired signals...")
    update_expired_signals(client)

    # Ambil semua data eksternal sekali di awal
    ...
    # Hard stop: volatility spike = kemungkinan event besar
    if vol_state == "SPIKE":
        tg(
            f"⚠️ <b>Volatility Spike</b>\n"
            f"ATR BTC sangat tinggi — kemungkinan ada event besar.\n"
            f"Scan ditunda. Regime: <b>{regime['regime']}</b>"
        )
        print("❌ Volatility spike — scan dihentikan")
        return

    if should_send_summary():
        send_market_summary(fg, fg_label, market_data, trending, regime)
    ...
    tg(
        ...
        f"━━━━━━━━━━━━━━━━━━\n"
        f"<i>Sinyal di atas pesan ini (jika ada)\n"
        f"sudah memenuhi semua threshold.</i>"
    )
