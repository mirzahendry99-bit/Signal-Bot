"""
╔══════════════════════════════════════════════════════════════════╗
║           SIGNAL BOT — CLEAN v7.16                              ║
║                                                                  ║
║  Perbaikan v7.16 (Next-Level Upgrade — 4 area):                 ║
║                                                                  ║
║  [v7.16 #A] Kelly Prior — Dynamic per Strategy + Regime         ║
║             Masalah: prior statis 2% untuk semua context        ║
║             PUMP TRENDING = SWING RANGING di mata bot           ║
║             Solusi: lookup table KELLY_PRIOR_BY_STRATEGY        ║
║             (strategy, regime) → prior fraction                 ║
║             PUMP+TRENDING: 6% | SWING+RANGING: 1.5%            ║
║             INTRADAY+TRENDING: 3% | MICROCAP: 2.5%             ║
║             get_kelly_prior(strategy, regime) → prior           ║
║             calc_position_size() terima strategy + regime       ║
║                                                                  ║
║  [v7.16 #B] Slippage — Live Spread + Order Book Depth          ║
║             Masalah: slippage flat 0.08% + volume_ratio         ║
║             tidak melihat spread aktual pair                    ║
║             Solusi 3 komponen:                                  ║
║             1. Half-spread live: get_live_spread() dari OB      ║
║                Gate.io — cache 5 menit                          ║
║             2. Market depth impact: get_ob_depth_impact()       ║
║                walk through 20 level OB → avg fill vs best      ║
║             3. Latency component (tetap proporsional volume)    ║
║             adjust_entry_for_slippage() terima client+pair      ║
║             Fallback: model statis jika client tidak tersedia   ║
║                                                                  ║
║  [v7.16 #C] Correlation — Pairwise Full Matrix + Dynamic Cluster║
║             Masalah: 3 cluster hardcoded (AI/MEME/L2)           ║
║             pair di luar cluster tidak terdeteksi               ║
║             Solusi: build_pairwise_matrix(client, pairs)        ║
║             • Fetch return series 1h+4h per pair (window=10)   ║
║             • Hitung pearson r semua pasangan (N×N matrix)      ║
║             • Dynamic clustering via transitive closure         ║
║               r > CORR_CLUSTER_THRESHOLD (0.70) = 1 cluster    ║
║             • Propagasi block: pair correlated ke dropper       ║
║               ikut diblok otomatis                              ║
║             • Cluster seed (AI/MEME/L2) tetap sebagai inisiasi  ║
║             is_cluster_blocked(): cek dynamic list DULU,        ║
║             baru seed cluster sebagai fallback                  ║
║                                                                  ║
║  [v7.16 #D] Equity — Fully Closed-Loop dari PnL Real           ║
║             Masalah: ACCOUNT_EQUITY_USDT = 200 statis           ║
║             sizing tidak pernah berubah meski equity naik/turun ║
║             Solusi: get_current_equity_usdt()                   ║
║             effective_equity = base + cumulative_pnl Supabase  ║
║             • Di-cache 30 menit (sinkron drawdown cache)        ║
║             • Floor: min 50% dari base (cegah under-sizing)     ║
║             • Di-pass ke calc_position_size() sebagai           ║
║               current_equity — override ACCOUNT_EQUITY_USDT    ║
║             • Tampil di Telegram summary setiap scan            ║
║             ACCOUNT_EQUITY_USDT tetap sebagai capital anchor    ║
║                                                                  ║
...
PORTFOLIO_STALE_HOURS  = 24   # signal > 24 jam tanpa result dianggap stale/closed
...
MIN_POSITION_USDT  =  5.0    # minimum agar order tidak rejected Gate.io
ACCOUNT_EQUITY_USDT  = 200.0  # [v7.14] total modal aktif — update berkala
TARGET_RISK_PCT      = 0.01   # [v7.14] risiko per trade = 1% dari equity
...
def _calc_equity_drawdown() -> float:
    ...
    # Hitung cumulative PnL dan running peak
    cumulative = 0.0
    peak       = 0.0
    for row in rows:
    ...
    if peak <= 0:
        return 0.0   # belum ada profit history → tidak bisa hitung DD dari peak
...
def save_signal(pair: str, strategy: str, side: str, entry: float,
    ...
    try:
        supabase.table("signals_v2").insert({
            ...
            "result":    None,
            "closed_at": None,  # [v7.12 #3] diisi oleh evaluate_open_trades()
        }).execute()
    except Exception as e:
...
def get_portfolio_state() -> dict:
    ...
    Returns:
        {"total": int, "buy": int, "sell": int}

    Fallback ke {"total": 0, "buy": 0, "sell": 0} jika Supabase
    tidak bisa di-reach — lebih aman lanjut daripada hard-block.
    ...
    try:
        ...
        return {"total": len(rows), "buy": buy_count, "sell": sell_count}
...
def evaluate_open_trades(client) -> dict:
    ...
    Returns:
        {"evaluated": int, "updated": int, "tp1": int, "tp2": int,
         "sl": int, "expired": int}
    """
    stats = {"evaluated": 0, "updated": 0, "tp1": 0, "tp2": 0, "sl": 0, "expired": 0}

    try:
        rows = (
            supabase.table("signals_v2")
            .select("id, pair, strategy, side, entry, tp1, tp2, sl, sent_at")
            .is_("result", "null")
    ...
    for row in rows:
        ...
        try:
            ...
            if result == "TP1_PARTIAL":
                ...
                update_payload = {"partial_result": "TP1_PARTIAL"}
            ...
            key = result.lower() if result in ("TP1", "TP2", "SL", "EXPIRED") else (
            ...
            emoji = {"TP2": "🎯🎯", "TP1": "🎯", "SL": "❌", "EXPIRED": "⏰",
                     "BREAKEVEN": "⚖️"}.get(result, "?")
            log(f"   {emoji} {result}: {pair} [{strategy} {side}]")
...
def build_equity_curve() -> dict:
    ...
    # ── Current drawdown ─────────────────────────────────────────────────
    current_dd_pct = ((peak - cumulative) / peak * 100) if peak > 0 else 0.0
...
def run():
    ...
    log(f"\n{'='*60}")
    log(f"🚀 SIGNAL BOT v7.16 — {datetime.now(WIB).strftime('%Y-%m-%d %H:%M WIB')} [FULL SCAN]")
    log(f"{'='*60}")
    ...
    if not signals and micro_sent == 0:
        tg(f"🔍 <b>Scan Selesai — SIGNAL BOT v7.16</b>\n"
    ...
    for sig in signals:
        ...
        sig["position_size"] = calc_position_size(
            ...
            current_equity=current_equity,   # [v7.16 #D] equity aktif, bukan statis
        )
