...
# [v9.2.1] Fixes:
#   - FIX 1: Hard-block entry deviation — check_intraday() dan check_swing()
#     return None jika |harga - entry| / entry > MAX_ENTRY_DEVIATION.
#     SWING: 5%, INTRADAY: 2%. Sebelumnya hanya warning teks, signal tetap dikirim.
#     Kasus UB/USDT +33.5% dari entry zone tidak akan lolos lagi.
#   - FIX 2: Adaptive price formatting — fmt_price() module-level helper.
#     Micro-price seperti PEPE sekarang tampil dengan desimal yang cukup sehingga
#     entry ≠ TP1 ≠ SL bisa dibaca jelas. Semua send_signal/pump/microcap diupdate.
#
# [v9.2 audit] Fixes (audit-driven):
BOT_VERSION = "9.2.1" # ← ubah satu baris ini saja saat rilis versi baru
...
# [v9.2 fix] Jika harga sudah terlalu jauh dari entry zone,
...
def fmt_price(v) -> str:
    """
    [v9.2 fix] Adaptive decimal formatting untuk harga crypto.
    Menampilkan cukup desimal agar entry ≠ TP1 ≠ SL pada micro-price (PEPE, dll).

    Contoh sebelumnya  : $0.000004 = $0.000004 = $0.000004  ← tidak terbaca
    Contoh sesudahnya  : $0.00000410, $0.00000472, $0.00000385  ← jelas
    """
    if v is None:
        return "—"
    fv = float(v)
    ...
    # Micro price: gunakan signifikan figure minimal 4 digit
    import math as _math
    mag = _math.floor(_math.log10(abs(fv)))  # misal 0.000004 → mag=-6
...
def check_intraday(client, pair: str, price: float,
    ...
    # [v9.2 fix] Hard-block: jika harga sudah terlalu jauh dari entry zone → skip.
    # Sebelumnya hanya warning teks di send_signal — signal tetap dikirim.
    _dev = abs(price - entry) / entry
    if _dev > MAX_ENTRY_DEVIATION["INTRADAY"]:
        log(f"   ⛔ INTRADAY SKIP {pair} [{side}] — "
            f"harga ${price:.8g} sudah {_dev*100:.1f}% dari entry zone ${entry:.8g} "
...
def check_swing(client, pair: str, price: float,
    ...
    # [v9.2 fix] Hard-block: harga sudah terlalu jauh dari entry zone → skip.
    _dev = abs(price - entry) / entry
...
def send_signal(sig: dict):
    ...
    # [v9.2 fix] Gunakan fmt_price() agar harga micro tidak terpotong
    entry_note = ""
    if side == "BUY":
        if pct_above > 0.5:
            entry_note = (
