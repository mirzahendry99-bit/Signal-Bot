"""
╔══════════════════════════════════════════════════════════════════╗
║           SIGNAL BOT — CLEAN v5.0                               ║
║                                                                  ║
║  Perbaikan dari v4.0 (4 bug kritis):                            ║
║                                                                  ║
║  [FIX #1] check_swing: hardcode side="BUY" → support BUY+SELL  ║
║           SELL path: BOS/CHoCH bearish + MACD + EMA + price     ║
║           di bawah EMA50 + late entry filter + RR check         ║
║                                                                  ║
║  [FIX #2] run(): already_sent dipanggil tanpa arg 'side'        ║
║           → TypeError crash saat runtime                         ║
║           Sekarang: already_sent(pair, strategy, side)           ║
║                                                                  ║
║  [FIX #3] run(): FG_SELL_BLOCK didefinisikan tapi tidak dipakai ║
║           → variabel mati, SELL tidak pernah diblokir/diaktifkan║
║           Sekarang: allow_sell = fg < FG_SELL_BLOCK              ║
║                     allow_buy  = not btc["block_buy"]            ║
║                                                                  ║
║  [FIX #4] send_signal: entry_note logic terbalik untuk SELL     ║
║           BUY: pct_above > 0.5 = harga sudah terlalu tinggi     ║
║           SELL: pct_above < -0.5 = harga sudah terlalu rendah   ║
║                                                                  ║
║  Arsitektur:                                                     ║
║  - INTRADAY (1h)  : BUY + SELL                                  ║
║  - SWING    (4h)  : BUY + SELL                                  ║
║  - PUMP SCANNER (15m) : BUY only                                ║
║  - Scoring murni teknikal: BOS/CHoCH + MACD + RSI + OB + Vol   ║
║  - RR minimum: INTRADAY 1.5 | SWING 2.0                        ║
║  - Dedup: tidak kirim ulang pair+strategy+side dalam 4 jam      ║
╚══════════════════════════════════════════════════════════════════╝
"""
...
# ── Volume & Pair Filter ──────────────────────────────
MIN_VOLUME_USDT    = 300_000     # diturunkan dari 500K — cover mid-cap altcoin
...
def score_signal(side: str, price: float, closes, highs, lows, volumes,
    ...
    if is_bull:
        ...
        if last_sl and price <= last_sl * 1.015: score += W["pullback"]
        prev = float(closes[-2])
        body  = price - prev
...
def already_sent(pair: str, strategy: str, side: str) -> bool:
    """
    Cek apakah signal pair+strategy+side sudah dikirim dalam DEDUP_HOURS jam terakhir.
    Side disertakan agar BUY dan SELL pada pair yang sama tidak saling memblokir.
    """
    try:
...
def check_intraday(client, pair: str, price: float, ob_ratio: float,
    ...
    if side == "BUY":
        ...
        if macd <= msig:   return None   # Gate 2: MACD bullish
        if rsi > 70:       return None   # Gate 3: tidak overbought
        if price < ema7:   return None   # Gate 4: di atas EMA7
        if ema7 < ema20:   return None   # Gate 5: short-term trend bullish
        ...
        has_struct = (structure.get("bos")   == "BEARISH" or
                      ...
                      liq.get("sweep_bear"))
        if not has_struct: return None
        if macd >= msig:   return None   # Gate 2: MACD bearish
        if rsi < 25:       return None   # Gate 3: tidak oversold ekstrem
        if price > ema7:   return None   # Gate 4: di bawah EMA7
        if ema7 > ema20:   return None   # Gate 5: short-term trend bearish
        ...
        last_sl = structure.get("last_sl")
        if last_sl and price < last_sl * 0.98: return None  # late entry filter
        entry = round(last_sl * 0.998, 8) if (last_sl and price < last_sl) else price
...
def check_swing(client, pair: str, price: float, ob_ratio: float,
    ...
    if side == "BUY":
        ...
        # Gate 3: price di atas EMA50 — trend jangka menengah bullish
        if price <= ema50: return None

        # Gate 4: RSI tidak overbought
        if rsi > 65: return None

        # Gate 5: price di atas EMA9 — momentum 4h positif
        if price < ema9: return None

        # Gate 6: EMA9 di atas EMA21 — konfirmasi trend 4h bullish
        ...
        # Entry: limit di retest BOS, > 2% di atas = terlambat
        last_sh = structure.get("last_sh")
    ...
    else:  # SELL — [FIX #1] path baru, mirror dari BUY dengan logika bearish
        # Gate 1: struktur bearish
        has_struct = (structure.get("bos")   == "BEARISH" or
                      structure.get("choch") == "BEARISH" or
                      liq.get("sweep_bear"))
        if not has_struct: return None
        ...
        # Gate 3: price di bawah EMA50 — trend jangka menengah bearish
        if price >= ema50: return None

        # Gate 4: RSI tidak oversold ekstrem
        if rsi < 30: return None

        # Gate 5: price di bawah EMA9 — momentum 4h negatif
        if price > ema9: return None

        # Gate 6: EMA9 di bawah EMA21 — konfirmasi trend 4h bearish
        if ema9 > ema21: return None
        ...
        # Entry: limit di retest BOS bearish, > 2% di bawah = terlambat
        last_sl = structure.get("last_sl")
        if last_sl and price < last_sl * 0.98: return None
        entry = round(last_sl * 0.998, 8) if (last_sl and price < last_sl) else price
...
def send_signal(sig: dict):
    ...
    msg = (
        ...
        f"TP2  : <b>${tp2:.6f}</b> <i>({tp_label}{pct_tp2:.1f}%)</i>\n"
...
def check_pump(client, pair: str, price: float, vol_24h: float) -> dict | None:
    ...
    Deteksi early pump berdasarkan:
      ...
      7. Anti buy-the-top: price tidak > 2% di atas high 3 candle terakhir
    """
    ...
    recent_high = float(np.max(highs[-4:-1]))
...
def run():
    ...
    print(f"\n{'='*60}")
    print(f"🚀 SIGNAL BOT v5.0 — {datetime.now(WIB).strftime('%Y-%m-%d %H:%M WIB')} [FULL SCAN]")
    print(f"{'='*60}")
    ...
    # [FIX #3] F&G gate sekarang benar-benar diaplikasikan ke alur scan
    allow_buy  = not btc["block_buy"]
    allow_sell = fg < FG_SELL_BLOCK

    print(f"Mode  : BUY={'✅ aktif' if allow_buy else '⛔ diblokir (BTC drop)'} | "
    ...
    for t in tickers:
        ...
        try:
            ...
            # ── INTRADAY BUY ──────────────────────────────────
            # [FIX #2] already_sent kini dipanggil dengan 3 argumen (pair, strategy, side)
            if allow_buy and not already_sent(pair, "INTRADAY", "BUY"):
                sig = check_intraday(client, pair, price, ob_ratio, btc, side="BUY")
                if sig: signals.append(sig)

            # ── INTRADAY SELL ─────────────────────────────────
            if allow_sell and not already_sent(pair, "INTRADAY", "SELL"):
                sig = check_intraday(client, pair, price, ob_ratio, btc, side="SELL")
                if sig: signals.append(sig)

            # ── SWING BUY ────────────────────────────────────
            if allow_buy and not already_sent(pair, "SWING", "BUY"):
                sig = check_swing(client, pair, price, ob_ratio, btc, side="BUY")
                if sig: signals.append(sig)

            # ── SWING SELL ───────────────────────────────────
            # [FIX #1] check_swing sekarang punya path SELL yang lengkap
            if allow_sell and not already_sent(pair, "SWING", "SELL"):
                sig = check_swing(client, pair, price, ob_ratio, btc, side="SELL")
                if sig: signals.append(sig)
    ...
    tg(f"🔍 <b>Scan Selesai — SIGNAL BOT v5.0</b>\n"
       f"━━━━━━━━━━━━━━━━━━\n"
    ...
    print(f"   INTRADAY BUY:{intraday_buy} SELL:{intraday_sell} | "
