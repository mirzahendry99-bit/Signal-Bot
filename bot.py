"""
╔══════════════════════════════════════════════════════════════════╗
║       SIGNAL BOT v8.4 — Bug Fix Edition                         ║
║                                                                  ║
...
def run():
    global _candle_cache, _dedup_memory, _4h_bias_cache
    ...
    log(f"\n{'='*60}")
    log(f"🚀 SIGNAL BOT v8.4 Bug Fix Edition — {datetime.now(WIB).strftime('%Y-%m-%d %H:%M WIB')} [FULL SCAN]")
    log(f"{'='*60}")
    ...
    if not signals and micro_sent == 0 and not momentum_signals:
        tg(f"🔍 <b>Scan Selesai — SIGNAL BOT v8.4 Bug Fix Edition</b>\n"
