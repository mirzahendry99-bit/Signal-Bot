...
BOT_VERSION = "9.4.3"  # ← ubah satu baris ini saja saat rilis versi baru
...
def _run_tests() -> None:
    ...
    assert_eq("INTRADAY adaptive bearish threshold = 3.0",
              _adaptive_intraday_bearish, 3.0)

    # Saat bearish_cycles < 2 → threshold normal 2.5
    _adaptive_intraday_normal = TIER_MIN_SCORE["A+"] + 0.0
    assert_eq("INTRADAY adaptive normal threshold = 2.5",
              _adaptive_intraday_normal, 2.5)

    # Saat bearish_cycles >= 2 → SWING threshold naik 3.0 → 3.5
    _adaptive_swing_bearish = TIER_MIN_SCORE_SWING + 0.5
    assert_eq("SWING adaptive bearish threshold = 3.5",
              _adaptive_swing_bearish, 3.5)
