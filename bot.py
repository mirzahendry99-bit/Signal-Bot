"""
╔══════════════════════════════════════════════════════════════════╗
║           SIGNAL BOT — CLEAN v7.26                              ║
║                                                                  ║
║  Perbaikan v7.26 (Invalid Pair Fast-Skip):                      ║
║                                                                  ║
║  [v7.26] gate_call_with_retry: skip retry jika 400              ║
        ...
        # Kirim Telegram alert saat 2 jam sebelum expired, satu kali saja.
            ...
            if partial_result == "TP1_PARTIAL":
                ...
                try:
                    ...
                    if _candles_trail and len(_candles_trail) >= 14:
                        ...
                        if _atr_trail and _atr_trail > 0:
                            if side == "BUY":
                                ...
                                _trail_sl = current_price + (_atr_trail * ATR_TRAIL_MULT)
                                # Ratchet: SL hanya turun, ceiling di entry (BE guaranteed)
                                _new_sl = min(sl, _trail_sl) if sl > 0 else _trail_sl
                                _new_sl = min(_new_sl, entry)
                    ...
                    pass  # trailing gagal → lanjut dengan SL lama (masih BE protected)
...
def run():
    ...
    if SCAN_MODE == "pump":
        # [v7.25] PUMP scan dinonaktifkan — fokus ke INTRADAY & SWING saja
        log("⏭️ PUMP scan dinonaktifkan [v7.25]")
        return
    ...
    log(f"\n{'='*60}")
    log(f"🚀 SIGNAL BOT v7.26 — {datetime.now(WIB).strftime('%Y-%m-%d %H:%M WIB')} [FULL SCAN]")
    log(f"{'='*60}")
    ...
    for t in tickers:
        ...
        try:
            ...
            if (allow_buy
                ...
                # [v7.25] MICROCAP dinonaktifkan — fokus ke INTRADAY & SWING
                pass
    ...
    if not signals and micro_sent == 0:
        tg(f"🔍 <b>Scan Selesai — SIGNAL BOT v7.26</b>\n"
           f"━━━━━━━━━━━━━━━━━━\n"
