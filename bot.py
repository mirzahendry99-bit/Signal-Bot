"""
╔══════════════════════════════════════════════════════════════════╗
║           SIGNAL BOT — CLEAN v3.0                               ║
║                                                                  ║
║  Perbaikan dari v2.1:                                           ║
║  - Entry berbasis RETEST ZONE (limit), bukan market price       ║
║  - BOS detection window: 3 → 5 candle (kurangi false positive)  ║
║  - Late entry filter diperketat: 3% → 2% untuk INTRADAY/SWING  ║
║  - Pullback score threshold diperketat: 3% → 1.5%              ║
║  - Pump scanner: +EMA20/EMA7 trend filter + anti buy-the-top    ║
║  - Telegram: tampilkan entry zone + peringatan jika harga jauh  ║
║                                                                  ║
...
def detect_structure(closes, highs, lows, strength=3, lookback=80) -> dict:
    ...
    # BOS: deteksi dalam 5 candle terakhir — cukup recent tanpa terlalu sensitif
    # 3 candle terlalu sempit → false positive tinggi pada setiap spike kecil
...
def check_intraday(client, pair: str, price: float, ob_ratio: float,
    ...
    # ── Entry Zone: limit di level retest BOS, bukan kejar harga ──────────
    # Jika harga sudah > 2% di atas last_sh → entry sudah terlambat, skip
    last_sh = structure.get("last_sh")
    if last_sh and price > last_sh * 1.02:
        return None

    # Entry optimal = sedikit di atas level BOS (retest zone)
    # Bukan harga saat ini — ini adalah limit order target
    if last_sh and price > last_sh:
        entry = round(last_sh * 1.002, 8)   # 0.2% di atas level BOS
    else:
        entry = price  # fallback jika last_sh tidak tersedia
    ...
    # Late signal filter — SL tidak boleh > 5% dari entry
    sl_dist = entry - sl
    ...
    return {
        ...
        "current_price": price,          # harga saat signal dibuat
...
def check_swing(client, pair: str, price: float, ob_ratio: float,
    ...
    # ── Entry Zone: limit di retest BOS, bukan market ─────────────────────
    # Swing lebih ketat: > 2% di atas last_sh sudah terlambat
    last_sh = structure.get("last_sh")
    if last_sh and price > last_sh * 1.02:
        return None

    if last_sh and price > last_sh:
        entry = round(last_sh * 1.003, 8)   # 0.3% di atas level BOS (sedikit lebih longgar untuk 4h)
    else:
        entry = price

    sl, tp1, tp2 = calc_sl_tp(entry, side, atr, structure, "SWING")

    if tp1 <= entry or sl >= entry: return None

    sl_dist = entry - sl
    if sl_dist <= 0: return None
    if sl_dist / entry > 0.10: return None
    rr = (tp1 - entry) / sl_dist
...
def send_signal(sig: dict):
    ...
    pct_above = (cur_price - entry) / entry * 100  # seberapa jauh harga dari entry zone

    tier_emoji  = {"S": "💎", "A+": "🏆", "A": "🥇"}.get(tier, "🎯")
    strat_emoji = {"INTRADAY": "📈", "SWING": "🌊"}.get(strategy, "🎯")
    side_emoji  = "🟢 BUY" if side == "BUY" else "🔴 SELL"

    # Notifikasi jika harga saat ini sudah di atas entry zone
    entry_note = ""
    if pct_above > 0.5:
...
def check_pump(client, pair: str, price: float, vol_24h: float) -> dict | None:
    ...
    # Gate 5 (NEW): EMA trend filter — harga harus di atas EMA20 pada 15m
    # Mencegah pump signal di tengah downtrend seperti RLS/USDT
    ema20_15m = calc_ema(closes, 20)
    if price < ema20_15m:
        return None

    # Gate 6 (NEW): EMA7 harus di atas EMA20 — konfirmasi momentum 15m positif
    ema7_15m = calc_ema(closes, 7)
    if ema7_15m < ema20_15m:
        return None

    # Gate 7 (NEW): Anti buy-the-top — harga tidak boleh > 2% di atas high 3 candle terakhir
    # Jika sudah spike jauh, bukan early pump lagi — terlambat
