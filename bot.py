"""
╔══════════════════════════════════════════════════════════════════╗
║        SIGNAL BOT — STABLE RELEASE v1.4                        ║
║                                                                  ║
║  Upgrade v1.4 — Intelligent Risk & Feedback Systems:           ║
║    NEW 1. Performance Feedback Loop                              ║
║           Winrate tracking, strategy ranking, KILL/SCALE auto   ║
║    NEW 2. Adaptive Thresholds (volatility + regime aware)        ║
║           PREPUMP_RSI_EXIT, ATR bands auto-adjust per market     ║
║    NEW 3. Capital Allocation per Strategy                        ║
║           SWING=40%, INTRADAY=30%, SCALPING=10%, SPEC=20%       ║
║    NEW 4. Equity Protection System                               ║
║           Daily max drawdown (-X%) → auto trading halt           ║
║                                                                  ║
...
print("🚀 SIGNAL BOT — STABLE v1.4 | Feedback Loop + Adaptive + Equity Protection")
...
# Alokasi slot maksimum per strategy bucket dalam satu cycle flush
MAX_SLOTS_CONFIRMED     = 3   # SWING + INTRADAY + SCALPING gabungan
MAX_SLOTS_SPECULATIVE   = 2   # MOONSHOT + PREPUMP + TRENDING gabungan
...
def flush_signal_queue(perf: dict, regime: dict):
    ...
    # [FIX 2] Sort by: Priority bucket → Tier → Score
    sorted_q = sorted(
        _signal_queue,
        key=lambda x: (
    ...
    print(f"\n📨 Queue: {len(sorted_q)} candidates → top {MAX_SIGNALS_PER_CYCLE} "
          f"(confirmed≤{MAX_SLOTS_CONFIRMED} | speculative≤{MAX_SLOTS_SPECULATIVE})")

    sent           = 0
    slots_confirmed    = 0   # [FIX 2] SWING + INTRADAY + SCALPING
    slots_speculative  = 0   # [FIX 2] MOONSHOT + PREPUMP + TRENDING
    ...
    for sig in sorted_q:
        ...
        # [FIX 4] Pass signal_type ke calc_position_size untuk speculative sizing
        ...
        # [FIX 4] Pass signal_type ke portfolio_allows untuk speculative cap
        ...
        rr      = round(pct_tp1/pct_sl,1) if pct_sl>0 else 0
        if rr < MIN_RR.get(signal_type,{}).get(tier,1.5):
        ...
        size_note = ""
        if risk_mult!=1.0:
            st = perf.get(strat_key,{}).get("status","")
        ...
        msg = (
            ...
            f"💰 Size: ${adj_size}{size_note} | Risk: {adj_risk}%\n"
            f"Data:   {sources}"
        )
        if sl_lines: msg += "\n━━━━━━━━━━━━━━━━━━\n🏗️ <b>Structure</b>\n" + "\n".join(sl_lines)
...
def run():
    ...
    tg(f"🔍 <b>Scan Selesai — STABLE v1.4</b>\n"
       f"━━━━━━━━━━━━━━━━━━\n"
       ...
       f"━━━━━━━━━━━━━━━━━━\n"
       f"[NEW 2] Adaptive | BTC ATR:{_adaptive['btc_atr_pct']:.2f}% | "
       f"RSI_exit:{_adaptive['prepump_rsi_exit']} | {'⚡ VOLATILE' if _adaptive['is_volatile'] else '😴 CALM'}\n"
       f"[NEW 4] Daily P&L: <b>{daily_pnl:+.2f}%</b> / -{DAILY_MAX_DRAWDOWN_PCT:.1f}% limit\n"
       f"━━━━━━━━━━━━━━━━━━\n"
       f"WATCHING sent : <b>{_watching_count}</b>\n"
       f"PrePump found : <b>{sum(1 for s in _signal_queue if s.get('signal_type')=='MOONSHOT_PREPUMP')}</b> 🔮\n"
       f"Trending Early: <b>{_trending_count[0]}</b> 🌟\n"
       f"Microcap      : <b>{_microcap_count[0]}</b> 🔬\n"
       f"━━━━━━━━━━━━━━━━━━\n"
       f"Portfolio Risk: <b>{_portfolio_state['total_risk_pct']:.1f}%</b>/{MAX_PORTFOLIO_RISK_PCT}%\n"
       f"Spec Risk     : <b>{_portfolio_state.get('speculative_risk_pct',0):.1f}%</b>/{MAX_SPECULATIVE_RISK_PCT}% "
       f"({'🔴 FULL' if _portfolio_state.get('speculative_risk_pct',0)>=MAX_SPECULATIVE_RISK_PCT else '🟢 OK'})\n"
       f"Slot used     : Confirmed <b>{_flush_slots['confirmed']}/{MAX_SLOTS_CONFIRMED}</b> | "
       f"Spec <b>{_flush_slots['speculative']}/{MAX_SLOTS_SPECULATIVE}</b>\n"
       f"━━━━━━━━━━━━━━━━━━\n"
       + "\n".join([
           f"[NEW 3] {k}: <b>{v:.2f}%</b> / {STRATEGY_MAX_RISK_PCT.get(k,0):.2f}%"
           for k, v in _portfolio_state.get("strategy_risk_pct", {}).items()
       ]) + "\n"
       f"━━━━━━━━━━━━━━━━━━\n"
