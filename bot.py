...
# ── Signal Quality Mode ───────────────────────
# CONSERVATIVE : sedikit sinyal, akurasi tinggi (MIN_STRENGTH=4, MIN_RR ketat)
# AGGRESSIVE   : lebih banyak sinyal, frekuensi tinggi (MIN_STRENGTH=3, MIN_RR longgar)
SIGNAL_MODE = "CONSERVATIVE"   # ganti ke "AGGRESSIVE" jika mau lebih banyak sinyal

MIN_STRENGTH = {"CONSERVATIVE": 4, "AGGRESSIVE": 3}[SIGNAL_MODE]

MIN_RR = {
    "CONSERVATIVE": {"SCALPING": 1.5, "INTRADAY": 1.8, "SWING": 2.5, "MOONSHOT": 2.0},
    "AGGRESSIVE":   {"SCALPING": 1.2, "INTRADAY": 1.5, "SWING": 2.0, "MOONSHOT": 1.8},
}[SIGNAL_MODE]

# ── Weighted Scoring ──────────────────────────
# Bobot kondisi berdasarkan kekuatan sinyal — bukan flat +1 semua
W = {
    # High-value signals (smart money / liquidity)
    "divergence":   3,   # RSI divergence — smart money reversal
    "liq_cluster":  3,   # Liquidation cluster — market magnet
    "oi_signal":    3,   # Open Interest signal — futures positioning
    # Medium-value signals (confirmation)
    "ichimoku":     2,   # Ichimoku cloud — structural trend
    "vwap":         2,   # VWAP bias — institutional fair value
    "bb_extreme":   2,   # Harga di BB extreme — overbought/oversold
    "funding":      2,   # Funding rate extreme — crowd positioning
    "poc":          2,   # Volume POC — akumulasi/distribusi
    # Standard signals (supporting)
    "rsi_extreme":  1,   # RSI overbought/oversold
    "stoch_rsi":    1,   # StochRSI confirmation
    "ema_cross":    1,   # EMA trend
    "ob_ratio":     1,   # Order book pressure
    "trending":     1,   # CoinGecko trending
    "fg_extreme":   1,   # Fear & Greed extreme
    "vol_spike":    1,   # Volume spike
    "support_res":  1,   # Support/Resistance zone
}

# Minimum total weighted score per mode
MIN_WSCORE = {
    "CONSERVATIVE": {"SCALPING": 7, "INTRADAY": 8, "SWING": 9,  "MOONSHOT": 7},
    "AGGRESSIVE":   {"SCALPING": 5, "INTRADAY": 6, "SWING": 7,  "MOONSHOT": 5},
}[SIGNAL_MODE]

# ── Volatility Filter (ATR-based) ─────────────
# Hindari entry di dead zone atau extreme spike
ATR_MIN_PCT = {   # ATR minimum sebagai % dari harga — hindari dead market
    "SCALPING": 0.3,
    "INTRADAY": 0.5,
    "SWING":    0.8,
}
ATR_MAX_PCT = {   # ATR maximum — hindari extreme volatility / news spike
    "SCALPING": 5.0,
    "INTRADAY": 8.0,
    "SWING":    12.0,
}
...
BTC_1H_DROP_BLOCK  = -2.0   # jika BTC turun >2% dalam 1h → block BUY alt
BTC_1H_PUMP_BLOCK  = 2.0    # jika BTC naik >2% dalam 1h → block SELL alt
