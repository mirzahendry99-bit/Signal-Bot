"""
╔══════════════════════════════════════════════════════════════════╗
║           SIGNAL BOT — CLEAN v2.0                               ║
║                                                                  ║
║  Arsitektur bersih dari nol:                                    ║
║  - INTRADAY (1h) + SWING (4h) only                             ║
...
║                                                                  ║
║  Jalan: setiap 4 jam via GitHub Actions                        ║
...
def detect_structure(closes, highs, lows, strength=3, lookback=80) -> dict:
    ...
    # BOS: break searah trend
    # CHoCH: break berlawanan trend (lebih kuat sebagai reversal signal)
    bull_break = price > last_sh and prev <= last_sh * 1.005
    bear_break = price < last_sl and prev >= last_sl * 0.995
...
def check_intraday(client, pair: str, price: float, ob_ratio: float,
    ...
    Tidak ada gate lain yang bisa memblokir jika 3 gate ini terpenuhi.
    """
...
def check_swing(client, pair: str, price: float, ob_ratio: float,
    ...
    Wajib: BOS/CHoCH + MACD + EMA50>EMA200 (trend filter HTF).
    """
    ...
    # Gate 3: EMA trend filter — EMA50 harus di atas EMA200 (uptrend)
    # Ini filter HTF paling penting untuk SWING — jangan entry melawan trend besar
    if ema50 <= ema200: return None
...
def run():
    ...
    print(f"\n{'='*60}")
    print(f"🚀 SIGNAL BOT v2.0 — {datetime.now(WIB).strftime('%Y-%m-%d %H:%M WIB')}")
    print(f"{'='*60}")
    ...
    print(f"\n📊 Scanned: {scanned} pairs | Candidates: {len(signals)}")

    if not signals:
        tg(f"🔍 <b>Scan Selesai — v2.0</b>\n"
    ...
    tg(f"🔍 <b>Scan Selesai — SIGNAL BOT v2.0</b>\n"
       f"━━━━━━━━━━━━━━━━━━\n"
