...
# ✅ FIX 1: R/R minimum dinaikkan — mencegah signal dengan profit kecil tapi risiko besar
MIN_RR = {
    "CONSERVATIVE": {"SCALPING": 1.8, "INTRADAY": 2.0, "SWING": 2.5, "MOONSHOT": 2.0},
    "AGGRESSIVE":   {"SCALPING": 1.5, "INTRADAY": 1.5, "SWING": 2.0, "MOONSHOT": 1.5},
}[SIGNAL_MODE]
...
# ✅ FIX 2: Order book hanya diambil untuk top pairs — percepat runtime
TOP_PAIRS_OB = {
    "BTC_USDT","ETH_USDT","SOL_USDT","BNB_USDT","XRP_USDT",
...
def get_order_book_pressure(client, pair):
    if pair not in TOP_PAIRS_OB:
        return 1.0  # neutral untuk pair lain
    try:
...
def get_market_regime(client) -> dict:
    ...
    try:
        closes_4h, highs_4h, lows_4h, _ = get_candles(client, "BTC_USDT", "4h", 210)
...
# ✅ FIX 3: Auto-update result signal yang sudah expired berdasarkan harga terakhir
def update_expired_signals(client):
    """
    Cek signal yang sudah expired dan update result (TP/SL/EXPIRED)
    berdasarkan harga terakhir saat signal valid berakhir.
    """
    try:
        now = datetime.now(WIB)
        ...
        for sig in res.data:
            ...
            current = float(ticker.last or 0)
            if current <= 0:
                continue
            ...
            print(f"📝 Update result [{pair}] {side}: {result} | Entry:{entry:.4f} Current:{current:.4f}")
...
def send_signal(pair, signal_type, side, entry, tp1, tp2, sl,
    ...
    # ✅ FIX 1: Filter R/R minimum
    min_rr = MIN_RR.get(signal_type, 1.5)
    if rr < min_rr:
        print(f"⏭️ Skip {pair} [{signal_type}] — R/R 1:{rr} < min 1:{min_rr}")
        return
...
def volatility_ok(atr: float, price: float, mode: str) -> tuple:
    ...
    if atr_pct < mn:
        return False, f"Volatilitas rendah ({atr_pct:.2f}%)"
    if atr_pct > mx:
        return False, f"Volatilitas terlalu tinggi ({atr_pct:.2f}%)"
    return True, ""
...
def check_scalping(client, pair, price, fg, ob_ratio, funding,
    ...
    if rsi < 35 and ema9 > ema21 and vol_ratio > 1.5:
        ...
        print(f"  📊 {pair} [SCALP BUY] wscore={score}/{min_wscore}")
        if score < min_wscore: return

        # ✅ FIX 1: TP lebih lebar untuk scalping
        tp1 = price * 1.05
    ...
    # ── SELL ─────────────────────────────────
        ...
        print(f"  📊 {pair} [SCALP SELL] wscore={score}/{min_wscore}")
        if score < min_wscore: return
...
def check_intraday(client, pair, price, fg, ob_ratio, funding,
    ...
    if rsi < 42 and macd > msig and price <= bb_mid:
        ...
        print(f"  📊 {pair} [INTRADAY BUY] wscore={score}/{min_wscore}")
        if score < min_wscore: return

        # ✅ FIX 1: TP lebih lebar — terbukti dari ONDO +5.7%
        ...
        send_signal(pair,"INTRADAY","BUY",price,tp1,tp2,sl,
                    stars_val,"1h",120,
                    "Gate.io · F&G · CoinGecko · StochRSI · VWAP · Ichimoku · Liq · OI",
                    extra)
    ...
    # ── SELL ─────────────────────────────────
        ...
        print(f"  📊 {pair} [INTRADAY SELL] wscore={score}/{min_wscore}")
        if score < min_wscore: return
...
def check_swing(client, pair, price, fg, ob_ratio, funding,
    ...
    if rsi < 50 and rsi > 20 and macd > msig:
        ...
        print(f"  📊 {pair} [SWING BUY] wscore={score}/{min_wscore}")
        if score < min_wscore: return
        ...
        send_signal(pair,"SWING","BUY",price,tp1,tp2,sl,
                    stars_val,"4h",720,
                    "Gate.io · F&G · CoinGecko · EMA50/200 · VWAP · Ichimoku · Liq · OI",
                    extra)
    ...
    # ── SELL ─────────────────────────────────
        ...
        print(f"  📊 {pair} [SWING SELL] wscore={score}/{min_wscore}")
        if score < min_wscore: return

        tp1 = max(price * 0.88, support * 1.02)
        tp2 = max(price * 0.78, support * 0.90)
...
def check_moonshot(client, pair, price, change_24h, trending, liq, regime):
    ...
    if vol_ratio > 4.0 and 25 < rsi < 70 and change_24h > 3.0:
        ...
        print(f"  📊 {pair} [MOONSHOT] wscore={score}/{MIN_WSCORE['MOONSHOT']} vol={vol_ratio:.1f}x")
        if score < MIN_WSCORE["MOONSHOT"]: return
...
def run():
    ...
    # ✅ FIX 3: Update hasil signal expired di awal run
    ...
    print(f"BTC Dom: {market_data.get('btc_dominance','N/A') if market_data else 'N/A'}% | F&G: {fg} | Vol: {vol_state}")

    if vol_state == "SPIKE":
        tg(
            f"⚠️ <b>Volatility Spike</b>\n"
            f"ATR BTC sangat tinggi — kemungkinan event besar.\n"
    ...
    for t in tickers:
        ...
        try:
            ...
            # Futures data hanya untuk top pairs
    ...
    tg(
        ...
        f"━━━━━━━━━━━━━━━━━━\n"
        f"<i>Sinyal di atas pesan ini (jika ada) sudah memenuhi\n"
        f"semua threshold kualitas yang ditetapkan.</i>"
    )
