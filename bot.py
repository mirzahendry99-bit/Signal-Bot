"""
╔══════════════════════════════════════════════════════════════════╗
║           SIGNAL BOT — SNIPER v7.8                              ║
║                                                                  ║
║  Upgrade dari v7.7 — SNIPER MODE:                               ║
║                                                                  ║
║  [v7.8 #1] TIER_MIN_SCORE diperketat: A:8→11, A+:10→13, S:14→16║
║            — hanya sinyal dengan konfirmasi sangat kuat lolos   ║
║  [v7.8 #2] MIN_RR diperketat: INTRADAY 1.5→2.0, SWING 2.0→2.5  ║
║            — setiap trade harus worth the risk                  ║
║  [v7.8 #3] SL lebih ketat: INTRADAY 1.5→1.3 ATR, SWING 2.0→1.8 ║
║            TP lebih besar: INTRADAY TP1 1.5→2.0, SWING TP1 2→2.5║
║  [v7.8 #4] trend_dir gate: ADX +DI/-DI sekarang digunakan      ║
║            sebagai hard gate — counter-trend signal di-reject   ║
║            BUY blok jika trend_dir BEARISH, SELL blok jika      ║
║            trend_dir BULLISH. NEUTRAL diperbolehkan.            ║
║  [v7.8 #5] HTF confirmation:                                    ║
║            INTRADAY (1h) → cek bias 4h tidak berlawanan         ║
║            SWING (4h) → cek bias 1d tidak berlawanan            ║
║  [v7.8 #6] BTC pump block (symmetrical):                        ║
║            SELL diblokir saat BTC naik > 3% dalam 1h           ║
║            — mirror dari BUY block saat BTC drop               ║
║  [v7.8 #7] MACD+EMA double-counter gate:                        ║
║            Jika MACD dan EMA keduanya berlawanan arah signal,   ║
║            signal di-reject (sebelumnya hanya penalti score)    ║
║  [v7.8 #8] BUY entry diubah ke pullback/support zone (last_sl)  ║
║            bukan post-breakout di atas last_sh — kurangi        ║
║            late entry dan buy-the-top                           ║
║  [v7.8 #9] MAX_SIGNALS_CYCLE: 8 → 6 (fokus kualitas)           ║
║                                                                  ║
║  Upgrade dari v6.0:                                             ║
║                                                                  ║
...
MAX_SIGNALS_CYCLE  = 6           # [v7.8 SNIPER] turun dari 8 → 6 — fokus kualitas
...
TIER_MIN_SCORE = {
    "S":  16,   # [v7.8 SNIPER] naik dari 14 → 16 — hanya sinyal sangat terkonfirmasi
    "A+": 13,   # [v7.8 SNIPER] naik dari 10 → 13
    "A":  11,   # [v7.8 SNIPER] naik dari 8 → 11 — filter keras untuk kualitas tinggi
}
...
# ── RR Minimum ───────────────────────────────────────
MIN_RR = {
    "INTRADAY": 2.0,   # [v7.8 SNIPER] naik dari 1.5 → 2.0 — hanya trade worth the risk
    "SWING":    2.5,   # [v7.8 SNIPER] naik dari 2.0 → 2.5
}
...
BTC_PUMP_BLOCK  =  3.0   # [v7.8] BTC naik > 3% dalam 1h → blok SELL baru (symmetrical)
...
# ── SL / TP Parameters ───────────────────────────────
INTRADAY_SL_ATR = 1.3    # [v7.8 SNIPER] SL lebih ketat: 1.5 → 1.3 (kurangi dollar risk)
INTRADAY_TP1_R  = 2.0    # [v7.8 SNIPER] TP1 naik: 1.5 → 2.0 (RR lebih baik)
INTRADAY_TP2_R  = 3.5    # [v7.8 SNIPER] TP2 naik: 2.5 → 3.5
SWING_SL_ATR    = 1.8    # [v7.8 SNIPER] SL lebih ketat: 2.0 → 1.8
SWING_TP1_R     = 2.5    # [v7.8 SNIPER] TP1 naik: 2.0 → 2.5
SWING_TP2_R     = 4.5    # [v7.8 SNIPER] TP2 naik: 3.5 → 4.5
...
def check_intraday(client, pair: str, price: float, ob_ratio: float,
    ...
    [v7.8 SNIPER] Gate tambahan:
    - trend_dir dari ADX harus searah signal (BUY → BULLISH/NEUTRAL, SELL → BEARISH/NEUTRAL)
    - HTF 4h bias harus tidak berlawanan dengan signal
    - BTC pump block: SELL diblokir saat BTC naik > 3% dalam 1h (symmetrical dengan BUY block)
    - BUY entry diubah ke pullback level (last_sl zone), bukan post-breakout
    """
    if side == "BUY"  and btc["block_buy"]:           return None
    if side == "SELL" and btc["btc_1h"] > BTC_PUMP_BLOCK: return None  # [v7.8] symmetrical guard
    ...
    # [v7.8 SNIPER] trend_dir gate — blok counter-trend signal
    # NEUTRAL diperbolehkan (pasar transisi), tapi BERLAWANAN langsung di-reject
    if side == "BUY"  and mkt["trend_dir"] == "BEARISH": return None
    if side == "SELL" and mkt["trend_dir"] == "BULLISH": return None

    # [v7.8 SNIPER] HTF 4h confirmation — bias 4h tidak boleh berlawanan
    htf_data = get_candles(client, pair, "4h", 60)
    if htf_data is not None:
        htf_mkt = detect_market_regime(htf_data[0], htf_data[1], htf_data[2])
        if side == "BUY"  and htf_mkt["trend_dir"] == "BEARISH": return None
        if side == "SELL" and htf_mkt["trend_dir"] == "BULLISH": return None
    ...
    if side == "BUY":
        ...
        # [v7.8 SNIPER] MACD dan EMA harus searah — tidak boleh keduanya berlawanan
        macd_bull = macd > msig
        ema_bull  = ema20 > ema50
        if not macd_bull and not ema_bull: return None  # keduanya counter = reject
        ...
        # [v7.8 SNIPER] Entry BUY diubah ke PULLBACK zone (last_sl area)
        # bukan post-breakout di atas last_sh — kurangi late entry & buy-the-top
        last_sl = structure.get("last_sl")
        last_sh = structure.get("last_sh")
        if last_sh and price > last_sh * 1.015: return None  # sudah terlalu jauh

        # Entry: di atas last_sl sedikit (support zone) — tunggu retest
        if last_sl and price <= last_sh:
            entry = round(last_sl * 1.003, 8)  # entry di support, bukan resistance
        else:
            entry = price  # fallback: harga saat ini
        ...
        if not macd_bear and not ema_bear: return None  # keduanya counter = reject
...
def check_swing(client, pair: str, price: float, ob_ratio: float,
    ...
    [v7.8 SNIPER] Gate tambahan:
    - trend_dir dari ADX 4h harus searah signal
    - HTF daily (1d) bias tidak boleh berlawanan
    - BTC pump block untuk SELL (symmetrical)
    - BUY entry diubah ke pullback/support zone
    """
    # [FIX #1] Blok BUY jika BTC drop — SELL tetap boleh jalan
    if side == "BUY"  and btc["block_buy"]:               return None
    if side == "SELL" and btc["btc_1h"] > BTC_PUMP_BLOCK: return None  # [v7.8] symmetrical
    ...
    # [v7.8 SNIPER] HTF Daily confirmation — bias 1d tidak boleh berlawanan
    htf_data = get_candles(client, pair, "1d", 60)
    if htf_data is not None:
        htf_mkt = detect_market_regime(htf_data[0], htf_data[1], htf_data[2])
        if side == "BUY"  and htf_mkt["trend_dir"] == "BEARISH": return None
        if side == "SELL" and htf_mkt["trend_dir"] == "BULLISH": return None
    ...
    if side == "BUY":
        ...
        if not macd_bull and not ema_bull: return None
        ...
        # [v7.8 SNIPER] Entry BUY diubah ke pullback/support zone
        last_sl = structure.get("last_sl")
        last_sh = structure.get("last_sh")
        if last_sh and price > last_sh * 1.015: return None  # terlalu jauh dari breakout

        if last_sl and price <= last_sh:
            entry = round(last_sl * 1.003, 8)  # entry di support zone
        else:
            entry = price
        ...
        if not macd_bear and not ema_bear: return None
        ...
        # [v7.2 FIX #4] Entry SELL di dekat last_sh (supply zone)
        entry = round(last_sh * 0.998, 8) if (last_sh and price >= last_sh * 0.97) else price
...
def run():
    ...
    if not signals and micro_sent == 0:
        tg(f"🔍 <b>Scan Selesai — SIGNAL BOT v7.8 SNIPER</b>\n"
