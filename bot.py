...
def check_intraday(client, pair: str, price: float, ob_ratio: float,
    ...
    if side == "BUY":
        ...
        # Root cause: bot overfit ke reversal engine → selalu cari SHORT saat harga naik.
        # Di bull market: ini = counter-trend = kalah terus.
        # Fix: jangan buka SELL jika BTC 4h >= +1% AND trend lokal ADX BULLISH.
...
def check_momentum(client, pair: str, price: float, ob_ratio: float,
    ...
    # ── Scoring ──────────────────────────────────────────
    score = 0
    score += W["momentum_break"]      # breakout sudah dikonfirmasi di Gate 2
    score += W["vol_confirm"]         # volume sudah dikonfirmasi di Gate 3

    if rsi >= 55:                     score += W["rsi_momentum"]   # RSI kuat di zona momentum
    if ema20 > ema50:                 score += W["ema_align"]       # EMA aligned
    if price > ema20:                 score += W["ema_align"]       # price di atas EMA20

    macd, msig = calc_macd(closes)
    if macd > msig:                   score += W["macd_cross"]      # MACD bullish
    elif macd < msig:                 score += W["macd_soft"]       # penalti counter
    ...
    # Volume spike kuat = bonus tambahan
    if vol_ratio >= 3.0:              score += 1

    # ADX bonus/penalty
    if mkt["regime"] == "TRENDING":   score += W["adx_trend"]
    elif mkt["regime"] == "RANGING":  score += W["adx_ranging"]
...
def run():
    ...
    log(f"\n{'='*60}")
    log(f"🚀 SIGNAL BOT v7.8 — {datetime.now(WIB).strftime('%Y-%m-%d %H:%M WIB')} [FULL SCAN]")
    log(f"{'='*60}")
    ...
    for t in tickers:
        ...
        try:
            ...
            # ── MOMENTUM BUY — [v7.8] trend continuation engine ──────
            # Dijalankan pada pair yang sudah lolos vol filter main bot.
            # Independen dari INTRADAY/SWING — fokus pada breakout & momentum,
            # bukan reversal. Menangkap koin yang "terbang" sebelum bot lama sadar.
            if allow_buy and not already_sent_momentum(pair):
                sig = check_momentum(client, pair, price, get_ob_ratio_lazy(pair), btc)
                if sig: momentum_signals.append(sig)
    ...
    if not signals and micro_sent == 0 and not momentum_signals:
        tg(f"🔍 <b>Scan Selesai — SIGNAL BOT v7.8</b>\n"
