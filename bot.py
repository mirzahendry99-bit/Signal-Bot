"""
╔══════════════════════════════════════════════════════════════════╗
║           SIGNAL BOT — CLEAN v8.0 (Audit Fix Complete)          ║
║                                                                  ║
...
def detect_structure(closes, highs, lows, strength=3, lookback=80) -> dict:
    ...
    bull_break = any(recent_closes[i] > last_sh and
                     recent_closes[i-1] <= last_sh * (1 + BOS_BREAK_TOLERANCE)
                     for i in range(1, len(recent_closes)))

    bear_break = any(recent_closes[i] < last_sl and
                     recent_closes[i-1] >= last_sl * (1 - BOS_BREAK_TOLERANCE)
                     for i in range(1, len(recent_closes)))
...
def get_btc_regime(client) -> dict:
    ...
    try:
        ...
        btc_bullish = chg_4h >= 1.0 and chg_1h >= 0.0
...
def evaluate_signals(client=None) -> None:
    ...
    for sig in pending:
        try:
            ...
            # Cek apakah harga pernah menyentuh TP/SL di candle manapun setelah entry
            # [v8.0 AUDIT FIX 5.2] Hapus dead code hit_tp2 kalkulasi pertama yang
            # langsung di-overwrite tanpa pernah dipakai. Hanya blok if/else di bawah
            # yang benar-benar digunakan untuk assignment akhir.
            if side == "BUY":
            ...
            if hit_tp2:
                ...
                result_val = "EXPIRED"
                current_price = float(data[0][-1])   # closes[-1]
...
def strategy_health_check() -> dict:
    ...
    try:
        for strat in strategies:
            ...
            health[strat] = {
                "win_rate": win_rate, "count": len(rows),
                "status": status, "strike": new_strike,
            }
            ...
            if is_crit:
                ...
                if new_strike >= HEALTH_CRITICAL_STRIKE_LIMIT:
                    tg(
                        ...
                        f"Win rate terbaru: <b>{win_rate}%</b> dari {len(rows)} trade.\n"
...
def check_intraday(client, pair: str, price: float, ob_ratio: float,
    ...
    Semua logika BUY/SELL identik dengan v7.9 — tidak ada perubahan behavior.
    """
...
def check_momentum(client, pair: str, price: float, ob_ratio: float,
    ...
    Menangkap trend continuation / early breakout.
    BUY only — tidak ada SELL mode karena engine ini fokus ikut arus.

    [v7.8 NEW] Solusi untuk: "koin terbang tapi bot diam."
    Bot lama terlalu banyak konfirmasi → entry terlambat → miss big move.
    Momentum engine ini deteksi SEBELUM semua konfirmasi align, tapi
    hanya ketika breakout + volume + momentum semua sepakat.
    """
    ...
    if closes[-1] <= recent_high * 1.001: return None   # belum breakout

    # Anti buy-the-top: jangan masuk jika sudah terlalu jauh di atas recent high
    if closes[-1] > recent_high * 1.04: return None   # sudah naik >4% dari breakout = terlambat
    ...
    # Volume spike kuat = bonus tambahan (di luar W_MOMENTUM karena skala berbeda)
    ...
    sl, tp1, tp2 = calc_sl_tp(entry, "BUY", atr, fake_struct, "INTRADAY")
...
def send_signal(sig: dict):
    ...
    # [v8.0 AUDIT FIX 5.1] Gunakan ENTRY_NOTE_WARN_PCT dan ENTRY_NOTE_READY_PCT
    # menggantikan magic numbers 0.5 dan 0.3
    entry_note = ""
    if side == "BUY":
        # BUY: harga naik terlalu jauh di atas entry = sudah terlambat
        if pct_above > ENTRY_NOTE_WARN_PCT:
            entry_note = (
...
def run():
    ...
    log(f"\n{'='*60}")
    log(f"🚀 SIGNAL BOT v7.9 — {datetime.now(WIB).strftime('%Y-%m-%d %H:%M WIB')} [FULL SCAN]")
    log(f"{'='*60}")
    ...
    for strat, info in health.items():
        if info.get("win_rate") is not None:
            log(f"  📊 Health [{strat}]: {info['win_rate']}% win rate "
                f"({info['count']} trades) — {info['status']}")
    ...
    def get_ob_ratio_micro(p: str) -> float:
        """Fetch ob_ratio (limit=20) untuk pair microcap/illiquid. Cache sekali per cycle."""
        if p not in _ob_ratio_cache_micro:
            _ob_ratio_cache_micro[p] = get_order_book_ratio(client, p, ob_limit=20)
        return _ob_ratio_cache_micro[p]
    ...
    for t in tickers:
        ...
        try:
            ...
            # MOMENTUM engine menangkap koin yang "terbang" sebelum INTRADAY/SWING sadar.
            # Diletakkan PERTAMA agar jika pair lolos Momentum, tidak di-double-signal
            # oleh INTRADAY yang mungkin juga valid pada pair yang sama.
            if allow_buy and pair not in _sent_pairs_this_cycle and not already_sent_momentum(pair):
                sig = check_momentum(client, pair, price, get_ob_ratio_lazy(pair), btc)
                if sig:
            ...
            # SELL bisa jalan meski pair sudah di-BUY oleh strategy lain —
            # sinyal SELL biasanya dari timeframe berbeda atau struktur yang berbeda.
    ...
    if not signals and micro_sent == 0 and not momentum_signals:
        tg(f"🔍 <b>Scan Selesai — SIGNAL BOT v7.9</b>\n"
