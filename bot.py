"""
╔══════════════════════════════════════════════════════════════════╗
║        SIGNAL BOT — STABLE RELEASE v1.0                        ║
║                                                                  ║
║  Dibangun berdasarkan SIGNAL_SPEC v1.0                          ║
║  Perubahan apapun harus mengacu ke spec sebelum dikode          ║
║                                                                  ║
...
def get_liquidation_bias(symbol: str) -> str:
    try:
        ...
        data = http_get(url, timeout=8)
        if not data or data.get("code") != "0": return "NEUTRAL"
        items     = data.get("data", {})
...
def check_moonshot(client, pair, price, change_24h, trending, liq_bias, regime):
    ...
    vol_avg   = np.mean(volumes[:-6]); vol_now = np.mean(volumes[-6:])
    vol_ratio = vol_now/vol_avg if vol_avg>0 else 0
    rsi       = calc_rsi(closes)
    if rsi>=70: return   # overbought — skip
...
def load_portfolio_state() -> dict:
    state = {"total_risk_pct":0.0,"open_positions":{},"sector_exposure":defaultdict(float)}
    try:
        ...
        print(f"📂 Portfolio: {len(state['open_positions'])} posisi | risk {state['total_risk_pct']:.1f}%")
    except Exception as e:
...
def is_valid(pair: str) -> bool:
    ...
    for sfx in ETF_STOCK_SUFFIXES:
        if base.endswith(sfx) and base!=sfx: return False
    return True
...
def save_signal(pair, signal_type, side, entry, tp1, tp2, sl,
    ...
    try:
        ...
        supabase.table("signals").insert({
            ...
            "tier":tier,"size_usdt":size_usdt,"risk_pct":risk_pct,
            ...
            "ob_valid":liq_map.get("equal_highs") is not None,
...
def flush_signal_queue(perf: dict, regime: dict):
    ...
    tier_ord = {"S":0,"A+":1,"A":2,"B":3}
    sorted_q = sorted(_signal_queue,
                      key=lambda x:(tier_ord.get(x.get("tier","B"),9),-x.get("score",0)))
    ...
    for sig in sorted_q:
        ...
        pair        = sig["pair"];    signal_type = sig["signal_type"]
        side        = sig["side"];    entry       = sig["entry"]
        tp1         = sig["tp1"];     tp2         = sig["tp2"]
        sl          = sig["sl"];      strength    = sig["strength"]
        timeframe   = sig["timeframe"]; tier      = sig["tier"]
        ...
        ok,reason = portfolio_allows(pair,side,tier,adj_risk,regime)
        if not ok:
        ...
        save_signal(pair,signal_type,side,entry,tp1,tp2,sl,
                    ...
                    structure,liq_map,precision)
        register_to_portfolio(pair,side,adj_risk)
...
def run():
    global _signal_queue, _watching_sent, _watching_count, _candle_cache, _portfolio_state
    ...
    client = get_client()
    print(f"=== SIGNAL SCAN | STABLE v1.0 ===")
    ...
    tg(f"🔍 <b>Scan Selesai — STABLE v1.0</b>\n"
       f"━━━━━━━━━━━━━━━━━━\n"
       ...
       f"━━━━━━━━━━━━━━━━━━\n"
       f"<i>Top {MAX_SIGNALS_PER_CYCLE} sinyal, ranked Tier → Score</i>")
