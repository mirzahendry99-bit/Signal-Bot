...
║  [v7.1 #1] Versi string konsisten di seluruh file (v7.1)        ║
...
║  [v7.3 #3] gate_call_with_retry: hapus dead code               ║
║            return None setelah loop (unreachable)               ║
...
def log(msg: str, level: str = "info"):
    """Log ke stdout dengan timestamp WIB."""
    ...
    if level == "warn":
        ...
        _logger.info(msg)
    # Tetap print agar output GitHub Actions terbaca langsung
    print(out)
...
def is_valid_pair(pair: str) -> bool:
    ...
    # [v7.2] Exact match untuk stablecoin — substring match menyebabkan false-positive
    # (mis. NDAI diblokir karena DAI, FRAXETH diblokir karena FRAX)
    # Leverage suffix tetap pakai substring karena 3S/5S memang suffix token leverage
    if base in BLACKLIST_TOKENS: return False
    if base in ETF_KEYWORDS: return False
...
def calc_bb(closes, period=20):
    """Bollinger Bands — saat ini unused, disimpan untuk pengembangan scoring berikutnya."""
    s = pd.Series(closes)
...
def detect_structure(closes, highs, lows, strength=3, lookback=80) -> dict:
    ...
    bull_break = any(recent_closes[i] > last_sh and
                     i > 0 and recent_closes[i-1] <= last_sh * 1.008
                     for i in range(len(recent_closes)))

    bear_break = any(recent_closes[i] < last_sl and
                     i > 0 and recent_closes[i-1] >= last_sl * 0.992
                     for i in range(len(recent_closes)))
...
def detect_order_block(closes, highs, lows, volumes, side="BUY", lookback=30) -> dict:
    ...
    # [v7.1 #4] Loop batas atas dikurangi 1 agar c[i+1] selalu valid
    for i in range(n - 3, 1, -1):   # i+1 maks = n-2 → selalu dalam range
        if i + 1 >= n: continue     # safety guard eksplisit
...
def already_sent(pair: str, strategy: str, side: str) -> bool:
    ...
    [v7.2 FIX #7] Gunakan UTC untuk konsistensi dengan Supabase (default UTC storage).
    """
    try:
...
def check_intraday(client, pair: str, price: float, ob_ratio: float,
    ...
    if side == "BUY":
        ...
        # bukan last_sl. Late entry filter: harga sudah terlalu jauh di bawah SH.
...
def already_sent_pump(pair: str) -> bool:
    ...
    """
    try:
        since = (datetime.now(timezone.utc) - timedelta(hours=PUMP_DEDUP_HOURS)).isoformat()
        res = supabase.table("signals_v2") \
            .select("id") \
            .eq("pair", pair) \
            .eq("strategy", "PUMP") \
        ...
        log(f"⚠️ dedup pump check: {e}", "warn")
        return False
...
def already_sent_micro(pair: str) -> bool:
    """Cek apakah microcap signal pair ini sudah dikirim dalam MICRO_DEDUP_HOURS jam.
    [v7.2 FIX #7] UTC konsisten dengan Supabase.
    """
    try:
        since = (datetime.now(timezone.utc) - timedelta(hours=MICRO_DEDUP_HOURS)).isoformat()
        res = supabase.table("signals_v2") \
            .select("id") \
            .eq("pair", pair) \
            .eq("strategy", "MICROCAP") \
            .gt("sent_at", since) \
            .execute()
        return len(res.data) > 0
    except Exception as e:
        log(f"⚠️ dedup micro check: {e}", "warn")
        return False
...
def check_microcap(client, pair: str, price: float,
    ...
    # Jika harga sudah terlalu jauh dari high 3 candle terakhir = terlambat
    recent_high = float(np.max(highs[-4:-1]))
...
def run():
    ...
    log(f"\n{'='*60}")
    log(f"🚀 SIGNAL BOT v7.4 — {datetime.now(WIB).strftime('%Y-%m-%d %H:%M WIB')} [FULL SCAN]")
    log(f"{'='*60}")
    ...
    for t in tickers:
        ...
        try:
            price      = float(t.last or 0)
            vol_24h    = float(t.quote_volume or 0)
            change_24h = float(t.change_percentage or 0) if t.change_percentage not in (None, "", "NaN") else 0.0  # [v7.2 FIX #6]
            if price <= 0: continue
            ...
            # [v7.2 FIX #10] ob_ratio di-fetch lazy — hanya saat ada kandidat yang perlu dicek.
            # Sebelumnya di-fetch per pair tanpa syarat → ratusan API calls per cycle → rate limit.
            # ob_ratio kontribusi kecil ke score (W=1), fetch hanya jika pair lolos dedup check.
            # Implementasi via list agar mutable di dalam fungsi nested (menghindari closure bug).
            _ob_cache = [False, 1.0]  # [fetched_flag, value]

            def get_ob_ratio_lazy(_pair=pair):
                """Fetch ob_ratio sekali saja per pair, cache hasilnya."""
                if not _ob_cache[0]:
                    _ob_cache[1] = get_order_book_ratio(client, _pair)
                    _ob_cache[0] = True
                return _ob_cache[1]
            ...
            if allow_buy and not already_sent(pair, "INTRADAY", "BUY"):
                sig = check_intraday(client, pair, price, get_ob_ratio_lazy(), btc, side="BUY")
                if sig: signals.append(sig)

            # ── INTRADAY SELL ─────────────────────────────────
            if allow_sell and not already_sent(pair, "INTRADAY", "SELL"):
                sig = check_intraday(client, pair, price, get_ob_ratio_lazy(), btc, side="SELL")
                if sig: signals.append(sig)

            # ── SWING BUY ────────────────────────────────────
            if allow_buy and not already_sent(pair, "SWING", "BUY"):
                sig = check_swing(client, pair, price, get_ob_ratio_lazy(), btc, side="BUY")
                if sig: signals.append(sig)

            # ── SWING SELL ───────────────────────────────────
            if allow_sell and not already_sent(pair, "SWING", "SELL"):
                sig = check_swing(client, pair, price, get_ob_ratio_lazy(), btc, side="SELL")
                if sig: signals.append(sig)
    ...
    tg(f"🔍 <b>Scan Selesai — SIGNAL BOT v7.4</b>\n"
       f"━━━━━━━━━━━━━━━━━━\n"
