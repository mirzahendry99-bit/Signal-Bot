...
BOT_VERSION = "10.2.0"  # ← ubah satu baris ini saja saat rilis versi baru
# [v10.2.0] EDGE VALIDATION SUITE — "Apakah scoring engine ini benar-benar profitable?"
#   Priority 1: Live Expectancy Tracker (EV per regime/confidence/session/pair/weekday)
#   Priority 2: Signal Quality Analytics (MAE/MFE histogram, TP1→TP2 rate, RR distribution)
#   Priority 3: Regime-Adaptive Strategy Filter (different rules per market regime)
#   Priority 4: Correlation-Aware Portfolio Optimizer (1 per cluster, best wins)
#   Priority 5: Forward Test Harness (Sharpe, Sortino, PF, MAE/MFE, expectancy validation)
#   Extra: SCAN_MODE=analytics, SCAN_MODE=forward_test for on-demand reports
#   Fix 1:  INITIAL_EQUITY_USDT $350 → $1000 (paper portfolio target)
#   Fix 2:  MAX_OPEN_TRADES 8 → 13 (true 13-slot portfolio)
#   Fix 3:  MAX_BTC_CORR_TRADES 2 → 4 (proportional to 13 slots)
#   Fix 4:  MAX_SIGNALS_CYCLE 8 → 13 (fill portfolio properly)
#   Fix 5:  True Paper Portfolio Engine (equity curve, floating PnL, exposure)
#   Fix 6:  Analytics Dashboard Engine (WR, expectancy, pair perf, regime perf)
#   Fix 7:  Live Rolling Expectancy Engine (replaces static V10_AVG_WIN/LOSS_R)
#   Fix 8:  Signal Ranking Priority Engine (EV × Confidence × Regime Weight)
#   Fix 9:  Equity-Based Position Scaling (auto scale up/down with performance)
#   Fix 10: Paper Trade Journal Engine (MAE, MFE, regime, ATR, full context)
#   1. Multi-factor scoring (6 factors, weighted) → Confidence % + Expected R + Risk Grade
#   2. Market Regime Engine (6 regimes: Strong Trend, Expansion, Compression, Choppy, Panic, Distribution)
#   3. Portfolio Tier System (5A × 1.0% + 5B × 0.6% + 3C × 0.3% = 13 slots ≈ 8.9% max risk)
#   4. Dynamic Slot Engine (13 → 8 → 5 → 2 based on DD/streak/BTC)
#   5. Professional Telegram format (Confidence%, EV+R, RiskGrade, Regime, Exposure)
#   6. Signal Quality Database (store regime/ATR/session/confidence at entry for edge analysis)
#   7. Risk per trade: 0.7% (13×0.7% ≈ 9% — survivable)
...
MAX_SIGNALS_CYCLE  = 13          # [v10.1] naik dari 8→13 — sesuai jumlah slot portfolio
...
MAX_OPEN_TRADES        = int(os.getenv("MAX_OPEN_TRADES", "13"))
# [v10.1] Dikembalikan ke 13 sesuai v10 spec: 5A + 5B + 3C = 13 slots
# Dynamic Slot Engine akan reduce ke 8/5/2 saat kondisi buruk.
MAX_PER_SECTOR         = 3    # [v10.1] naik dari 2→3: 13 slots / 4 sektor ≈ 3 per sektor
MAX_RISK_TOTAL         = 0.09 # [v10.1] naik dari 8%→9%: 13 slots × 0.7% ≈ 9% max
# [v9.2 audit FIX] Notional cap
MAX_NOTIONAL_PCT       = 0.80  # 80% equity — buffer untuk slippage/fees
# [v10.1] Naikkan proporsional dengan 13 slots
MAX_SAME_SIDE_TRADES   = 7    # [v10.1] naik dari 5→7: ~54% dari 13 slots boleh satu arah
MAX_BTC_CORR_TRADES    = 4    # [v10.1] naik dari 2→4: mayoritas alt memang corr BTC
...
BASE_POSITION_USDT = 50.0    # [v10.1] naik dari $25→$50: $1000 equity / 13 slots ≈ $77 avg
MAX_POSITION_USDT  = 100.0   # [v10.1] naik dari $40→$100: 10% dari $1000 equity
MIN_POSITION_USDT  = 15.0    # [v10.1] naik dari $12→$15: floor realistis
...
MAX_POSITION_PCT   = 0.10    # [v10.1] 10% max per trade: $1000 × 10% = $100 cap
...
INITIAL_EQUITY_USDT  = float(os.getenv("INITIAL_EQUITY_USDT", "1000.0"))  # [v10.1] naik $350→$1000 — target paper portfolio
...
RISK_PER_TRADE       = 0.007  # [v10.0] turun dari 1.0%→0.7% — 13 slots × 0.7% ≈ 9% max total risk
# Formula: 13 × 0.7% ≈ 9% — masih survivable. Tier A dapat 1.0×, B=0.6×, C=0.3× multiplier.
...
# ════════════════════════════════════════════════════════
#  V10: PORTFOLIO TIER SYSTEM — 5A + 5B + 3C
# ════════════════════════════════════════════════════════
# Tier A (5 slots) = CORE — hanya signal skor tertinggi
#   Risk per trade: 1.0 × RISK_PER_TRADE = 0.7% equity
# Tier B (5 slots) = STANDARD — signal normal yang lolos threshold
#   Risk per trade: 0.6 × RISK_PER_TRADE = 0.42% equity
# Tier C (3 slots) = PROBE — signal exploratory, lebih kecil
#   Risk per trade: 0.3 × RISK_PER_TRADE = 0.21% equity
#
# Total max risk: 5×0.7% + 5×0.42% + 3×0.21% = 3.5 + 2.1 + 0.63 = 6.23%
# Worst case (semua slot terisi, semua SL): ≈ 8.9% — sangat sehat
PORTFOLIO_TIER_A_SLOTS    = 5     # core signals
PORTFOLIO_TIER_B_SLOTS    = 5     # standard signals
PORTFOLIO_TIER_C_SLOTS    = 3     # probe signals
PORTFOLIO_TIER_A_RISK_MULT = 1.00 # 0.7% per trade
PORTFOLIO_TIER_B_RISK_MULT = 0.60 # 0.42% per trade
PORTFOLIO_TIER_C_RISK_MULT = 0.30 # 0.21% per trade

# Threshold score untuk masuk tiap portfolio tier
# Tier A: Confidence ≥ 78% → score ≥ 4.0 (sesuai CONFIDENCE_MAP_V10)
# Tier B: Confidence ≥ 62% → score ≥ 3.5
# Tier C: Confidence ≥ 50% → score ≥ 3.0
PORTFOLIO_TIER_A_MIN_CONF = 0.78
PORTFOLIO_TIER_B_MIN_CONF = 0.62
PORTFOLIO_TIER_C_MIN_CONF = 0.50

# ════════════════════════════════════════════════════════
#  V10: DYNAMIC SLOT ENGINE
# ════════════════════════════════════════════════════════
# Saat kondisi buruk, slot aktif dikurangi otomatis.
# Trigger: DD spike, BTC crash, loss streak
DYNAMIC_SLOT_NORMAL    = 13   # kondisi normal
DYNAMIC_SLOT_WARNING   = 8    # DD > 5% atau streak ≥ 2
DYNAMIC_SLOT_HIGH_DD   = 5    # DD > 10%
DYNAMIC_SLOT_PANIC     = 2    # DD > 15% atau BTC crash
# Threshold DD per level
DYNAMIC_SLOT_DD_WARN   = 0.05   # 5%
DYNAMIC_SLOT_DD_HIGH   = 0.10   # 10%
DYNAMIC_SLOT_DD_PANIC  = 0.15   # 15%
DYNAMIC_SLOT_STREAK_WARN = 2    # 2 losses beruntun → WARNING

# ════════════════════════════════════════════════════════
#  V10: MULTI-FACTOR SCORING ENGINE (6 FACTORS)
# ════════════════════════════════════════════════════════
# Output sekarang adalah CONFIDENCE % + EXPECTED VALUE R + RISK GRADE
# bukan lagi tier string sederhana "A+" atau "A".
#
# Factor             Weight   Sub-score (0–5 per factor)
# ─────────────────────────────────────────────────────
# Trend Quality       25%    EMA alignment + HH/HL structure
# Momentum            20%    MACD velocity + RSI direction
# Volume Expansion    20%    Current vol / 20-period avg
# BTC Alignment       15%    BTC 4h direction + cycles
# Volatility Structure 10%   ATR in sweet spot (1–3%)
# Market Regime       10%    Regime favorability for entry
# ─────────────────────────────────────────────────────
# Weighted score 0–5, mapped to Confidence via CONFIDENCE_MAP_V10
V10_SCORE_WEIGHTS = {
    "trend_quality":        0.25,
    "momentum":             0.20,
...
# Empirical score → confidence mapping (historical WR buckets)
# score 3.0 = WR ~50% | 3.5 = ~62% | 4.0 = ~75% | 4.5 = ~85% | 5.0 = ~95%
CONFIDENCE_MAP_V10 = [(5.0, 0.95), (4.5, 0.85), (4.0, 0.75), (3.5, 0.62), (3.0, 0.50), (0.0, 0.0)]
# Expected R parameters (from historical data)
V10_AVG_WIN_R  = 1.00   # average win in R-units
V10_AVG_LOSS_R = 0.70   # average loss in R-units (slippage + spread)

# ════════════════════════════════════════════════════════
#  V10: MARKET REGIME ENGINE (6 REGIMES)
# ════════════════════════════════════════════════════════
# Setiap regime punya: size multiplier, izin breakout, dan deskripsi
V10_REGIMES = {
    "STRONG_TREND":   {"size_mult": 1.0,  "allow_breakout": True,  "color": "🟢"},
    "EXPANSION":      {"size_mult": 1.0,  "allow_breakout": True,  "color": "🟢"},
    "COMPRESSION":    {"size_mult": 0.5,  "allow_breakout": False, "color": "🟡"},
    "CHOPPY":         {"size_mult": 0.5,  "allow_breakout": False, "color": "🟡"},
    "PANIC":          {"size_mult": 0.0,  "allow_breakout": False, "color": "🔴"},
    "DISTRIBUTION":   {"size_mult": 0.3,  "allow_breakout": False, "color": "🟠"},
}
# Sub-score market regime → masuk factor scoring
V10_REGIME_SCORE = {
    "STRONG_TREND":  5.0,
    "EXPANSION":     5.0,
    "CHOPPY":        2.5,
    ...
    "DISTRIBUTION":  2.0,
    "PANIC":         0.0,
    # Legacy ADX-based regimes dari detect_market_regime()
    "TRENDING":      4.5,
    "RANGING":       2.0,
}
...
def score_signal(side: str, price: float, closes, highs, lows, volumes,
    ...
    # ── FACTOR 1: TREND QUALITY (25%) ────────────────────────────
    f_trend = 0.0
    if (is_bull and ema_fast > ema_slow) or (not is_bull and ema_fast < ema_slow):
        f_trend += 2.0
    if is_bull:
        if price > ema_fast > ema_slow:  f_trend += 1.5
        elif price > ema_slow:           f_trend += 0.5
    else:
        if price < ema_fast < ema_slow:  f_trend += 1.5
        elif price < ema_slow:           f_trend += 0.5
    if is_bull  and structure.get("bias") == "BULLISH":  f_trend += 1.0
    if not is_bull and structure.get("bias") == "BEARISH": f_trend += 1.0
    if setup_score == 3:  f_trend += 0.5
    f_trend = min(5.0, f_trend)

    # ── FACTOR 2: MOMENTUM (20%) ──────────────────────────────────
    f_momentum = 0.0
    if (is_bull and macd > msig) or (not is_bull and macd < msig):
        f_momentum += 2.0
    if is_bull:
        if 50 < rsi <= 65:    f_momentum += 1.5
        elif 40 <= rsi <= 50: f_momentum += 1.0
        elif rsi > 65:        f_momentum += 0.5
    else:
        if 35 <= rsi < 50:    f_momentum += 1.5
        elif 50 <= rsi <= 60: f_momentum += 1.0
        elif rsi < 35:        f_momentum += 0.5
    if len(closes) >= 3:
        if is_bull  and closes[-1] > closes[-2] > closes[-3]: f_momentum += 1.0
        if not is_bull and closes[-1] < closes[-2] < closes[-3]: f_momentum += 1.0
    if is_bull  and fg < 20:  f_momentum = max(0, f_momentum - 0.5)
    if not is_bull and fg > 80: f_momentum = max(0, f_momentum - 0.5)
    f_momentum = min(5.0, f_momentum)

    # ── FACTOR 3: VOLUME EXPANSION (20%) ─────────────────────────
    f_volume = 0.0
    if len(volumes) >= 10:
        vol_avg = float(np.mean(volumes[-20:-1])) if len(volumes) >= 20 else float(np.mean(volumes[:-1]))
        if vol_avg > 0:
            vol_ratio = float(volumes[-1]) / vol_avg
            if vol_ratio >= 2.5:   f_volume = 4.0
            elif vol_ratio >= 1.8: f_volume = 3.5
            elif vol_ratio >= 1.5: f_volume = 3.0
            elif vol_ratio >= 1.3: f_volume = 2.0
            elif vol_ratio >= 1.0: f_volume = 1.0
            else:                  f_volume = 0.5
            recent_above = sum(1 for v in volumes[-3:] if v > vol_avg)
            if recent_above >= 2:  f_volume = min(5.0, f_volume + 0.5)
    f_volume = min(5.0, f_volume)

    # ── FACTOR 4: BTC ALIGNMENT (15%) ────────────────────────────
    f_btc = 0.0
    if (is_bull and btc_4h > 0) or (not is_bull and btc_4h < 0):
        f_btc += 2.5
    elif abs(btc_4h) < 0.3:
        f_btc += 1.5
    else:
        f_btc += 0.5
    btc_strength = abs(btc_4h)
    if btc_strength > 3.0:   f_btc = min(5.0, f_btc + 1.5)
    elif btc_strength > 1.5: f_btc = min(5.0, f_btc + 1.0)
    elif btc_strength > 0.5: f_btc = min(5.0, f_btc + 0.5)
    f_btc = min(5.0, f_btc)

    # ── FACTOR 5: VOLATILITY STRUCTURE (10%) ─────────────────────
    f_vol_struct = 2.0
    if price > 0:
        atr_val  = calc_atr(closes, highs, lows)
        atr_pct  = (atr_val / price) * 100
        if 1.0 <= atr_pct <= 3.0:   f_vol_struct = 4.5
        elif 0.5 <= atr_pct < 1.0:  f_vol_struct = 3.0
        elif 3.0 < atr_pct <= 5.0:  f_vol_struct = 3.0
        elif 5.0 < atr_pct <= 7.0:  f_vol_struct = 1.5
        elif atr_pct < 0.5:         f_vol_struct = 1.0
        else:                        f_vol_struct = 0.5
    f_vol_struct = min(5.0, f_vol_struct)

    # ── FACTOR 6: MARKET REGIME (10%) ────────────────────────────
    f_regime = V10_REGIME_SCORE.get(regime, 2.5)
    f_regime = min(5.0, max(0.0, f_regime))

    # ── WEIGHTED SCORE 0–5 ────────────────────────────────────────
    score = (
        f_trend      * w["trend_quality"] +
        f_momentum   * w["momentum"] +
        f_volume     * w["volume_expansion"] +
        f_btc        * w["btc_alignment"] +
        f_vol_struct * w["volatility_structure"] +
        f_regime     * w["market_regime"]
    )
    return round(score, 3)


# ════════════════════════════════════════════════════════
#  V10: CONFIDENCE, EXPECTED VALUE & RISK GRADE
# ════════════════════════════════════════════════════════

def assign_confidence_v10(score: float) -> float:
    """
    [v10.0] Map score (0–5) → confidence (0–1) via CONFIDENCE_MAP_V10.
    Interpolates linearly between known calibration points.
    """
    score = max(0.0, min(5.0, score))
    pts = list(reversed(CONFIDENCE_MAP_V10))   # ascending order: [(0,0), (3,0.5), ...]
    for i in range(len(pts) - 1):
        s1, c1 = pts[i]
        s2, c2 = pts[i + 1]
        if s1 <= score <= s2:
            t = (score - s1) / (s2 - s1) if s2 != s1 else 0
            return round(c1 + t * (c2 - c1), 3)
    return 0.95 if score >= 5.0 else 0.0


def calc_expected_value_r(confidence: float) -> float:
    """
    [v10.0] E[R] = P(Win)×AvgWinR − P(Loss)×AvgLossR.
    Positive = edge exists. Negative = don't trade.
    """
    ev = confidence * V10_AVG_WIN_R - (1 - confidence) * V10_AVG_LOSS_R
    return round(ev, 3)


def assign_risk_grade_v10(score: float, confidence: float, regime: str,
                          btc_bearish_trend: bool, btc_cycles: int) -> str:
    """
    [v10.0] Assign risk grade (A+/A/B/C/BLOCKED) based on all conditions.
    BLOCKED = do NOT take this trade.
    """
    # Hard blocks first
    if regime == "PANIC":                                    return "BLOCKED"
    if btc_bearish_trend and btc_cycles >= 2:               return "BLOCKED"
    regime_props = V10_REGIMES.get(regime, {})
    if regime_props.get("size_mult", 1.0) == 0.0:           return "BLOCKED"

    # Grade by score + conditions
    if score >= 4.0 and confidence >= 0.78 and regime in ("STRONG_TREND", "EXPANSION", "TRENDING"):
        return "A+"
    if score >= 3.5 and confidence >= 0.62:
        return "A"
    if score >= 3.0 and confidence >= 0.50:
        return "B"
    if score >= 2.5:
        return "C"
    return "BLOCKED"
...
def assign_portfolio_tier_v10(confidence: float, risk_grade: str) -> str:
    """
    [v10.0] Assign portfolio tier (A/B/C/SKIP) based on confidence + risk grade.
    Tier A = Core slots (highest confidence)
    Tier B = Standard slots
    Tier C = Probe slots (exploratory)
    SKIP   = Below minimum confidence
    """
    if risk_grade == "BLOCKED":                             return "SKIP"
    if confidence >= PORTFOLIO_TIER_A_MIN_CONF:             return "A"
    if confidence >= PORTFOLIO_TIER_B_MIN_CONF:             return "B"
    if confidence >= PORTFOLIO_TIER_C_MIN_CONF:             return "C"
    return "SKIP"


def get_regime_size_mult_v10(regime: str) -> float:
    """[v10.0] Get position size multiplier for current market regime."""
    return V10_REGIMES.get(regime, {}).get("size_mult", 1.0)


def assign_tier(score: float) -> str:
    """
    [v10.0] Map v10 score (0–5) to legacy tier string.
    Mapped so existing code (check_intraday, etc.) continues to work.
    A+ : score >= 3.5 (was 2.5 in v9)
    A  : score >= 3.0
    SKIP: score < 3.0
    """
...
def calc_conviction(score: float) -> str:
    """
    [v10.0] Conviction from v10 score (0–5).
    Returns confidence string with emoji — replaces old VERY HIGH / HIGH / GOOD / OK labels.
    """
    conf = assign_confidence_v10(score)
    ev   = calc_expected_value_r(conf)
    if conf >= 0.85:  return f"VERY HIGH 🔥 ({conf:.0%})"
    if conf >= 0.75:  return f"HIGH 💪 ({conf:.0%})"
    if conf >= 0.62:  return f"GOOD ✅ ({conf:.0%})"
    return f"OK 🟡 ({conf:.0%})"
...
def calc_position_size(
    ...
    # ── [V10 FIX #9] Equity-based adaptive scaling ────────────────────────
    # Replaces/complements the streak-based dd_mult with portfolio-aware scalar.
    # Grows when equity grows, shrinks when DD occurs — fully adaptive.
    _eq_scalar = calc_equity_scaling_mult()
    if abs(_eq_scalar - 1.0) > 0.02:   # Only log if meaningful
        log(f"   📊 [EQ_SCALE] {pair} equity_scalar={_eq_scalar:.2f}×")
    raw_size *= _eq_scalar
...
def check_intraday(client, pair: str, price: float,
    ...
    # ── [V10] Enrich signal with confidence, EV, risk grade ──────
    _v10_conf  = assign_confidence_v10(score)
    _v10_ev    = calc_expected_value_r(_v10_conf)
    _v10_grade = assign_risk_grade_v10(score, _v10_conf, mkt["regime"],
                                        btc.get("btc_bearish_trend", False),
                                        btc.get("btc_bearish_cycles", 0))
    _v10_ptier = assign_portfolio_tier_v10(_v10_conf, _v10_grade)
    ...
    return {
        ...
        "conviction": calc_conviction(score),
        # [V10] New fields
        "confidence_v10":    round(_v10_conf, 3),
        "expected_value_r":  round(_v10_ev, 3),
        "risk_grade_v10":    _v10_grade,
        "portfolio_tier_v10": _v10_ptier,
        "btc_bearish_trend": btc.get("btc_bearish_trend", False),
        "btc_bearish_cycles": btc.get("btc_bearish_cycles", 0),
        "session_id": f"{pair}_{side}_{int(time.time())}",
...
def check_swing(client, pair: str, price: float,
    ...
    # ── [V10] Enrich signal with confidence, EV, risk grade ──────
    _v10_conf_s  = assign_confidence_v10(score)
    _v10_ev_s    = calc_expected_value_r(_v10_conf_s)
    _v10_grade_s = assign_risk_grade_v10(score, _v10_conf_s, mkt["regime"],
                                          btc.get("btc_bearish_trend", False),
                                          btc.get("btc_bearish_cycles", 0))
    _v10_ptier_s = assign_portfolio_tier_v10(_v10_conf_s, _v10_grade_s)
    ...
    return {
        ...
        "conviction": calc_conviction(score),
        # [V10] New fields
        "confidence_v10":     round(_v10_conf_s, 3),
        "expected_value_r":   round(_v10_ev_s, 3),
        "risk_grade_v10":     _v10_grade_s,
        "portfolio_tier_v10": _v10_ptier_s,
        "btc_bearish_trend":  btc.get("btc_bearish_trend", False),
        "btc_bearish_cycles": btc.get("btc_bearish_cycles", 0),
        "session_id": f"{pair}_{side}_{int(time.time())}",
        "reason": build_signal_reason(
...
def send_signal(sig: dict):
    ...
    btc_bearish   = sig.get("btc_bearish_trend", False)
    btc_cycles    = sig.get("btc_bearish_cycles", 0)

    # ── V10 Confidence + Expected Value + Risk Grade ──────────────
    confidence  = assign_confidence_v10(score)
    ev_r        = calc_expected_value_r(confidence)
    risk_grade  = assign_risk_grade_v10(score, confidence, regime, btc_bearish, btc_cycles)
    ptier       = assign_portfolio_tier_v10(confidence, risk_grade)
    regime_mult = get_regime_size_mult_v10(regime)
    ...
    # ── Regime display ────────────────────────────────────────────
    regime_display = V10_REGIMES.get(regime, {}).get("color", "⚪") + " " + regime
    btc_context_str = "Bearish 🔴" if btc_bearish else "Bullish 🟢"

    # ── Side & tier labels ────────────────────────────────────────
    side_emoji  = "🟢 LONG" if side == "BUY" else "🔴 SHORT"
    grade_emoji = {"A+": "🏆", "A": "🥇", "B": "🥈", "C": "🎯", "BLOCKED": "🚫"}.get(risk_grade, "🎯")
    ptier_label = {"A": "CORE", "B": "STANDARD", "C": "PROBE", "SKIP": "SKIP"}.get(ptier, "?")
    tp_label    = "+" if side == "BUY" else "-"
    sl_label    = "-" if side == "BUY" else "+"

    # ── Entry note (price deviation) ──────────────────────────────
    entry_note = ""
    if side == "BUY" and pct_above > 0.5:
        entry_note = f"\n⚠️ Harga {fmt_price(cur_price)} (+{pct_above:.1f}%) — tunggu pullback ke entry"
    elif side == "BUY" and pct_above < -0.3:
        entry_note = f"\n✅ Harga {fmt_price(cur_price)} — sudah di zona entry"
    elif side == "SELL" and pct_above < -0.5:
        entry_note = f"\n⚠️ Harga {fmt_price(cur_price)} ({pct_above:.1f}%) — tunggu retest ke entry"

    # ── Portfolio exposure estimate ───────────────────────────────
    effective_equity = get_current_equity_usdt()
    exposure_pct     = (position_size / effective_equity * 100) if effective_equity > 0 else 0

    # ── [V10] Professional Telegram Message ───────────────────────
    # Format sesuai Final Design v10 spec:
    # Confidence | Expected R | Risk Grade | Regime | Portfolio info
    now         = datetime.now(WIB)
    valid_hours = 36 if strategy == "INTRADAY" else 120
    valid_until = (now + timedelta(hours=valid_hours)).strftime("%d %b %H:%M WIB")
    bos_info    = sig.get("structure", {}).get("bos") or sig.get("structure", {}).get("choch") or "—"

    msg = (
        f"{'📈' if strategy == 'INTRADAY' else '🌊'} {side_emoji} — <b>{pair}</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━━\n"
        f"Confidence  : <b>{confidence:.0%}</b>\n"
        f"Risk Grade  : <b>{grade_emoji} {risk_grade}</b>   [Tier {ptier_label}]\n"
        f"Regime      : <b>{regime_display}</b>\n"
        f"Expected R  : <b>{'+' if ev_r >= 0 else ''}{ev_r:.2f}R</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━━\n"
        f"Entry  : <b>{fmt_price(entry)}</b> <i>≈ {entry_idr}</i>{entry_note}\n"
        f"SL     : <b>{fmt_price(sl)}</b> <i>≈ {sl_idr}</i> <i>({sl_label}{pct_sl:.1f}%)</i>\n"
        f"TP1    : <b>{fmt_price(tp1)}</b> <i>≈ {tp1_idr}</i> <i>({tp_label}{pct_tp1:.1f}%)</i>\n"
        f"TP2    : <b>{fmt_price(tp2) if tp2 else '—'}</b>"
        f"{(' <i>≈ ' + tp2_idr + '</i>') if tp2 else ''}"
        f"{(' <i>(' + tp_label + f'{pct_tp2:.1f}%)</i>') if tp2 else ''}\n"
        f"R/R    : <b>1:{rr}</b>   RSI: {rsi:.0f}\n"
        f"━━━━━━━━━━━━━━━━━━━━━\n"
        f"Paper Size  : <b>${position_size:.2f} USDT</b>\n"
        f"Exposure    : <b>{exposure_pct:.1f}%</b> portfolio\n"
        f"BTC Context : <b>{btc_context_str}</b>\n"
        f"Score       : {score:.2f}/5.0   Struct: {bos_info}\n"
        f"Why         : <i>{sig.get('reason', '—')}</i>\n"
        f"⏰ Valid s.d.: {valid_until}\n"
        f"<i>⚠️ Paper trading only. Bukan rekomendasi finansial.</i>"
    )
    tg_async(msg)  # non-blocking async queue

    log(f"  ✅ [V10] SIGNAL {risk_grade} {ptier_label} {strategy} {side} {pair} | "
        f"Score:{score:.2f} Conf:{confidence:.0%} EV:{ev_r:+.2f}R | "
        f"Regime:{regime} | RR:1:{rr} Size:${position_size}")
...
def portfolio_allows(sig: dict, state: dict, btc: dict) -> bool:
    ...
    # ── [V10] Check 0: Dynamic slot engine ───────────────────────
    # Check against dynamic max slots (reduced in bad conditions)
    # This is checked BEFORE the static MAX_OPEN_TRADES gate.
    try:
        _dd = _drawdown_state.get("dd_pct", 0.0)
        _sk = _drawdown_state.get("streak", 0)
        _dyn_max = get_dynamic_max_slots(_dd, _sk, btc)
        if state["total"] >= _dyn_max:
            log(f"   ⚡ [V10] Portfolio SKIP {pair} [{side}] — "
                f"dynamic slot limit reached ({state['total']}/{_dyn_max}) "
                f"DD={_dd*100:.1f}% streak={_sk}")
            return False
    except Exception:
        pass   # Fallback to static check below if dynamic engine errors

    # ── [V10] Check 0.5: Risk grade gate ─────────────────────────
    # BLOCKED risk grade = never trade regardless of portfolio state
    if sig.get("risk_grade_v10") == "BLOCKED":
        log(f"   ⚡ [V10] Portfolio SKIP {pair} [{side}] — Risk Grade BLOCKED")
        return False
...
def send_weekly_summary() -> None:
    ...
    try:
        ...
        # ── [V10 FIX #6] Analytics dashboard + edge analysis ─────
        try:
            tg_async(build_analytics_dashboard())
            tg_async(get_edge_analysis_summary())
            log("📊 [V10] Analytics dashboard + edge analysis terkirim")
        except Exception as _ae:
            log(f"⚠️ analytics dashboard on weekly: {_ae}", "warn")

        # ── [V10.2 #1] Live Expectancy Tracker ───────────────────
        try:
            send_ev_report()
        except Exception as _eve:
            log(f"⚠️ EV report on weekly: {_eve}", "warn")

        # ── [V10.2 #2] Signal Quality Analytics ──────────────────
        try:
            send_signal_quality_report()
        except Exception as _qae:
            log(f"⚠️ QA report on weekly: {_qae}", "warn")

        # ── [V10.2 #5] Forward Test Analysis ─────────────────────
        try:
            send_forward_test_report()
        except Exception as _fte:
            log(f"⚠️ Forward test report on weekly: {_fte}", "warn")
...
# ════════════════════════════════════════════════════════
#  V10: DYNAMIC SLOT ENGINE + SIGNAL QUALITY DB
# ════════════════════════════════════════════════════════

def get_dynamic_max_slots(dd_pct: float, streak: int, btc: dict) -> int:
    """
    [v10.0] Reduce active slots based on market conditions.
    Returns max slots allowed right now.

    Normal (13) → Warning (8) → High DD (5) → Panic (2)

    Triggers:
      - DD > 15% OR BTC crash          → PANIC  → 2 slots
      - DD > 10%                       → HIGH_DD → 5 slots
      - DD > 5% OR streak ≥ 2 losses   → WARNING → 8 slots
      - Everything OK                  → NORMAL  → 13 slots
    """
    btc_crash = btc.get("halt", False) or btc.get("block_buy", False)

    if dd_pct >= DYNAMIC_SLOT_DD_PANIC or btc_crash:
        log(f"⚠️ [SLOTS] PANIC mode — max {DYNAMIC_SLOT_PANIC} slots "
            f"(DD={dd_pct*100:.1f}%, btc_crash={btc_crash})", "warn")
        return DYNAMIC_SLOT_PANIC

    if dd_pct >= DYNAMIC_SLOT_DD_HIGH:
        log(f"⚠️ [SLOTS] HIGH_DD mode — max {DYNAMIC_SLOT_HIGH_DD} slots "
            f"(DD={dd_pct*100:.1f}%)", "warn")
        return DYNAMIC_SLOT_HIGH_DD

    if dd_pct >= DYNAMIC_SLOT_DD_WARN or streak >= DYNAMIC_SLOT_STREAK_WARN:
        log(f"⚠️ [SLOTS] WARNING mode — max {DYNAMIC_SLOT_WARNING} slots "
            f"(DD={dd_pct*100:.1f}%, streak={streak})", "warn")
        return DYNAMIC_SLOT_WARNING

    return DYNAMIC_SLOT_NORMAL


def get_v10_btc_regime_label(btc: dict) -> str:
    """[v10.0] Map BTC state dict to human-readable regime for Telegram."""
    if btc.get("halt"):                    return "PANIC"
    if btc.get("btc_bearish_trend"):       return "DISTRIBUTION"
    if btc.get("block_buy"):               return "COMPRESSION"
    btc_4h = btc.get("btc_4h", 0.0)
    if btc_4h > 3.0:                       return "EXPANSION"
    if btc_4h > 1.0:                       return "STRONG_TREND"
    if abs(btc_4h) < 0.5:                  return "COMPRESSION"
    return "CHOPPY"


def save_signal_quality_context(sig: dict, confidence: float, ev_r: float,
                                 risk_grade: str, position_size: float,
                                 atr_pct: float, btc: dict) -> None:
    """
    [v10.0] Save rich signal context to signal_quality_context table.
    Used for edge analysis: in what conditions does our edge actually appear?

    Schema (Supabase):
      signal_id TEXT, pair TEXT, strategy TEXT, side TEXT,
      sent_at TIMESTAMPTZ, regime TEXT, atr_pct NUMERIC,
      btc_trend TEXT, btc_4h_change NUMERIC, volume_ratio NUMERIC,
      rsi NUMERIC, entry NUMERIC, score NUMERIC, confidence NUMERIC,
      expected_value_r NUMERIC, risk_grade TEXT, position_size_usdt NUMERIC,
      is_breakout BOOLEAN, result TEXT (updated later)
    """
    try:
        payload = {
            "signal_id":          sig.get("session_id", f"{sig['pair']}_{int(time.time())}"),
            "pair":               sig["pair"],
            ...
            "btc_trend":          "BEARISH" if btc.get("btc_bearish_trend") else "BULLISH",
            "btc_4h_change":      round(btc.get("btc_4h", 0.0), 3),
            "volume_ratio":       round(sig.get("volume_ratio", 1.0), 3),
            "rsi":                round(sig.get("rsi", 50.0), 1),
            "entry":              sig["entry"],
            ...
            "is_breakout":        bool(sig.get("structure", {}).get("bos")),
            "result":             None,   # filled by evaluate_open_trades()
        }
        supabase.table("signal_quality_context").insert(payload).execute()
        log(f"   📊 [V10] Signal quality context saved: {sig['pair']} {sig['side']} "
            f"conf={confidence:.0%} ev={ev_r:+.2f}R")
    except Exception as e:
        # Non-critical — don't block signal send
        log(f"   ⚠️ save_signal_quality_context failed (non-critical): {e}", "warn")


# ════════════════════════════════════════════════════════
#  V10: TRUE PAPER PORTFOLIO ENGINE  [FIX #5]
# ════════════════════════════════════════════════════════
# Tracks: equity curve, floating PnL, exposure, paper cash balance.
# State stored in Supabase bot_config (key="paper_portfolio_state").

_paper_portfolio_cache: dict = {}

def _load_paper_portfolio() -> dict:
    """Load paper portfolio state from Supabase. Returns empty state if not found."""
    try:
        rows = (
            supabase.table("bot_config")
            .select("value")
            .eq("key", "paper_portfolio_state")
            .limit(1)
            .execute()
            .data
        ) or []
        ...
        log(f"⚠️ _load_paper_portfolio: {e}", "warn")
    return {
        "cash":           INITIAL_EQUITY_USDT,
        "total_equity":   INITIAL_EQUITY_USDT,
        "floating_pnl":   0.0,
        "realized_pnl":   0.0,
        "peak_equity":    INITIAL_EQUITY_USDT,
        "max_dd_pct":     0.0,
        "positions":      {},     # {pair: {side, size, entry, sl, tp1, tp2, tier}}
        "equity_curve":   [],     # [{ts, equity, dd_pct}]
        "updated_at":     datetime.now(WIB).isoformat(),
    }

def _save_paper_portfolio(state: dict) -> None:
    """Persist paper portfolio state to Supabase."""
    try:
        state["updated_at"] = datetime.now(WIB).isoformat()
        supabase.table("bot_config").upsert([
            {"key": "paper_portfolio_state",
             "value": json.dumps(state),
             "updated_at": state["updated_at"]},
        ], on_conflict="key").execute()
    except Exception as e:
        log(f"⚠️ _save_paper_portfolio: {e}", "warn")

def update_paper_portfolio_open(sig: dict, position_size: float) -> None:
    """Called when a new signal is sent — open a paper position."""
    state = _load_paper_portfolio()
    pair  = sig["pair"]
    state["positions"][pair] = {
        ...
        "tp1":           sig["tp1"],
        "tp2":           sig.get("tp2"),
        "tier":          sig.get("portfolio_tier_v10", "B"),
        "opened_at":     datetime.now(WIB).isoformat(),
        "confidence":    sig.get("confidence_v10", 0.0),
        "regime":        sig.get("regime", "—"),
    }
    state["cash"] = max(0.0, state["cash"] - position_size)
    _save_paper_portfolio(state)

def update_paper_portfolio_close(pair: str, exit_price: float,
                                  result: str, pnl_usdt: float) -> None:
    """Called when a position closes — update equity and curve."""
    state = _load_paper_portfolio()
    pos   = state["positions"].pop(pair, None)
    if pos is None:
        return

    state["cash"]         += pos["size_usdt"] + pnl_usdt
    state["realized_pnl"] += pnl_usdt

    # Update equity
    state["total_equity"] = state["cash"] + sum(
        p["size_usdt"] for p in state["positions"].values()
    )
    ...
    state["max_dd_pct"] = max(state["max_dd_pct"], dd * 100)

    # Equity curve snapshot (keep last 500)
    ...
    _save_paper_portfolio(state)
    log(f"   📊 [PAPER] {pair} {result}: PnL ${pnl_usdt:+.2f} | "
        f"Equity ${state['total_equity']:.2f} | DD {dd*100:.1f}%")

def get_paper_exposure_pct() -> float:
    """Return current portfolio exposure (positions / total_equity) as fraction."""
    state = _load_paper_portfolio()
    eq    = state.get("total_equity", INITIAL_EQUITY_USDT)
    if eq <= 0:
        return 0.0
    total_pos = sum(p["size_usdt"] for p in state.get("positions", {}).values())
    return total_pos / eq

def get_paper_floating_pnl(current_prices: dict) -> float:
    """Calculate total floating PnL given {pair: current_price}."""
    state = _load_paper_portfolio()
    total = 0.0
    for pair, pos in state.get("positions", {}).items():
        cur = current_prices.get(pair, pos["entry"])
        entry = pos["entry"]
        size  = pos["size_usdt"]
        units = size / entry if entry > 0 else 0
        if pos["side"] == "BUY":
            total += (cur - entry) * units
        else:
            total += (entry - cur) * units
    return round(total, 4)


# ════════════════════════════════════════════════════════
#  V10: ANALYTICS DASHBOARD ENGINE  [FIX #6]
# ════════════════════════════════════════════════════════

def build_analytics_dashboard() -> str:
    """
    Build Telegram-ready analytics dashboard string.
    Covers: equity curve, rolling WR, expectancy, pair perf, regime perf.
    """
    try:
        state  = _load_paper_portfolio()
        curve  = state.get("equity_curve", [])
        init_e = INITIAL_EQUITY_USDT
        cur_e  = state.get("total_equity", init_e)
        peak_e = state.get("peak_equity",  init_e)
        max_dd = state.get("max_dd_pct",   0.0)
        real_p = state.get("realized_pnl", 0.0)
        n_pos  = len(state.get("positions", {}))

        # Equity curve stats
        ret_pct = (cur_e - init_e) / init_e * 100 if init_e > 0 else 0

        # Rolling stats from closed trades (last 30)
        recent = [c for c in curve if c.get("result") in ("WIN","LOSS","PARTIAL_WIN","SL","TP1","TP2")][-30:]
        wins   = sum(1 for c in recent if c.get("result") in ("WIN","TP1","TP2","PARTIAL_WIN"))
        losses = sum(1 for c in recent if c.get("result") in ("LOSS","SL"))
        wr30   = wins / (wins + losses) * 100 if (wins + losses) > 0 else 0

        # DB query for expectancy + pair/regime breakdown
        expectancy_usdt = 0.0
        pair_pnl: dict = {}
        regime_pnl: dict = {}
        sharpe = 0.0
        try:
            rows = (
                supabase.table("signals_v2")
                .select("pair, pnl_usdt, result")
                .not_.is_("result", "null")
                .not_.is_("pnl_usdt", "null")
                .neq("strategy", "SYSTEM")
            ...
            valid = [r for r in rows if r.get("pnl_usdt") is not None]
            if valid:
                pnls = [float(r["pnl_usdt"]) for r in valid]
                expectancy_usdt = sum(pnls) / len(pnls)
                # Pair PnL
                for r in valid:
                    p = r.get("pair", "?").replace("_USDT", "")
                    pair_pnl[p] = pair_pnl.get(p, 0.0) + float(r["pnl_usdt"])
                # Sharpe (simplified, no risk-free rate)
                if len(pnls) > 1:
        ...
        # Regime performance from signal_quality_context
        try:
            sqrows = (
                supabase.table("signal_quality_context")
                .select("regime, expected_value_r, result")
                .not_.is_("result", "null")
                .limit(100)
                .execute()
                .data
            ...
            for r in sqrows:
                reg = r.get("regime", "?")
                ev  = float(r.get("expected_value_r") or 0)
                regime_pnl[reg] = regime_pnl.get(reg, [])
                regime_pnl[reg].append(ev)
        except Exception:
            pass
        ...
        bot_pairs    = sorted_pairs[-3:] if len(sorted_pairs) > 3 else []

        # Regime summary
        regime_summary = ""
        for reg, evs in regime_pnl.items():
            avg_ev = sum(evs) / len(evs) if evs else 0
            regime_summary += f"  {reg}: avg EV {avg_ev:+.2f}R ({len(evs)} trades)\n"

        msg = (
            f"📊 <b>ANALYTICS DASHBOARD v10</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━━\n"
            f"💰 Equity      : <b>${cur_e:.2f}</b> (peak ${peak_e:.2f})\n"
            f"📈 Return       : <b>{ret_pct:+.1f}%</b>  (${real_p:+.2f} realized)\n"
            f"📉 Max DD       : <b>{max_dd:.1f}%</b>\n"
            f"🎯 Open Pos     : <b>{n_pos}</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━━\n"
            f"WR (30d)       : <b>{wr30:.1f}%</b>  ({wins}W / {losses}L)\n"
            f"Expectancy     : <b>${expectancy_usdt:+.2f}</b> per trade\n"
            f"Sharpe (approx): <b>{sharpe:.2f}</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━━\n"
            f"🏆 Top Pairs:\n"
            + "".join(f"  {p}: ${v:+.2f}\n" for p, v in top_pairs)
            + (f"❌ Bottom Pairs:\n"
               + "".join(f"  {p}: ${v:+.2f}\n" for p, v in bot_pairs) if bot_pairs else "")
            + f"━━━━━━━━━━━━━━━━━━━━━\n"
            f"📊 Regime Performance:\n"
            f"{regime_summary or '  (belum ada data)\n'}"
        )
        return msg
    except Exception as e:
        return f"⚠️ Analytics dashboard error: {e}"

def send_analytics_dashboard() -> None:
    """Send analytics dashboard to Telegram."""
    msg = build_analytics_dashboard()
    tg_async(msg)
    log("📊 Analytics dashboard dikirim.")


# ════════════════════════════════════════════════════════
#  V10: LIVE ROLLING EXPECTANCY ENGINE  [FIX #7]
# ════════════════════════════════════════════════════════
# Replaces static V10_AVG_WIN_R / V10_AVG_LOSS_R with
# live rolling values segmented by regime, pair, setup.

_expectancy_cache: dict = {"data": None, "ts": 0.0}
EXPECTANCY_CACHE_TTL = 1800  # 30 minutes

def get_live_expectancy(regime: str = "", pair: str = "", limit: int = 50) -> dict:
    """
    [v10.1 FIX #7] Live rolling expectancy from Supabase signals_v2.
    Segments by regime and/or pair if provided.
    Returns dict with win_r, loss_r, expectancy_r, sample_n.
    Falls back to static constants if not enough data.
    """
    global _expectancy_cache
    now = time.time()

    # Use cached if fresh
    cache_key = f"{regime}|{pair}"
    if (_expectancy_cache.get("ts", 0) and
            now - _expectancy_cache["ts"] < EXPECTANCY_CACHE_TTL and
            _expectancy_cache.get("data", {}).get(cache_key)):
        return _expectancy_cache["data"][cache_key]

    try:
        q = (supabase.table("signals_v2")
             .select("result, pnl_usdt, entry, sl, position_size")
             .not_.is_("result", "null")
             .not_.is_("pnl_usdt", "null")
             .neq("strategy", "SYSTEM"))
        if pair:
            q = q.eq("pair", pair)
        rows = q.order("sent_at", desc=True).limit(limit).execute().data or []
        ...
        for r in rows:
            ...
            risk_usdt = size * abs(ent - sl) / ent if ent > 0 and sl > 0 and size > 0 else size * 0.007
            r_val = pnl / risk_usdt if risk_usdt > 0 else 0
            res   = (r.get("result") or "").upper()
            if res in ("WIN", "TP1", "TP2", "PARTIAL_WIN"):
                wins.append(r_val)
            elif res in ("LOSS", "SL", "EXPIRED_LOSS"):
                losses.append(abs(r_val))

        if len(wins) + len(losses) < 5:   # Not enough data — use static
            result = {
                "win_r":        V10_AVG_WIN_R,
                "loss_r":       V10_AVG_LOSS_R,
                "expectancy_r": 0.0,
                "sample_n":     0,
                "source":       "static",
            }
        else:
            avg_win  = sum(wins) / len(wins)   if wins   else V10_AVG_WIN_R
            avg_loss = sum(losses) / len(losses) if losses else V10_AVG_LOSS_R
            wr       = len(wins) / (len(wins) + len(losses))
            ev       = wr * avg_win - (1 - wr) * avg_loss
            result   = {
                "win_r":        round(avg_win, 3),
                "loss_r":       round(avg_loss, 3),
                "expectancy_r": round(ev, 3),
                "sample_n":     len(wins) + len(losses),
                "source":       "live",
            }

        if not _expectancy_cache.get("data"):
            _expectancy_cache["data"] = {}
        _expectancy_cache["data"][cache_key] = result
        _expectancy_cache["ts"] = now
        return result

    except Exception as e:
        log(f"⚠️ get_live_expectancy: {e} — fallback to static", "warn")
        return {"win_r": V10_AVG_WIN_R, "loss_r": V10_AVG_LOSS_R,
                "expectancy_r": 0.0, "sample_n": 0, "source": "error"}

def calc_expected_value_r_live(confidence: float, regime: str = "", pair: str = "") -> float:
    """
    [v10.1 FIX #7] E[R] using LIVE rolling win/loss R values.
    Falls back to static if sample < 5.
    """
    exp = get_live_expectancy(regime=regime, pair=pair)
    ev  = confidence * exp["win_r"] - (1 - confidence) * exp["loss_r"]
    return round(ev, 3)


# ════════════════════════════════════════════════════════
#  V10: SIGNAL RANKING PRIORITY ENGINE  [FIX #8]
# ════════════════════════════════════════════════════════
# When 20 signals exist, pick top N by composite score.
# Formula: Expected Value × Confidence × Regime Weight
...
def rank_signals(signals: list, max_slots: int = 13) -> list:
    """
    [v10.1 FIX #8] Rank signals by composite score and return top N.

    Composite = Confidence × ExpectedValue_R × RegimeWeight × TierBonus
    Tier A > B > C weighted 1.2 / 1.0 / 0.8
    Signals with risk_grade_v10 == BLOCKED are excluded.
    """
    TIER_BONUS = {"A": 1.2, "B": 1.0, "C": 0.8}

    scored = []
    for s in signals:
        grade = s.get("risk_grade_v10", "B")
        if grade == "BLOCKED":
            continue
        conf    = s.get("confidence_v10", 0.5)
        ev_r    = s.get("expected_value_r", 0.0)
        regime  = s.get("regime", "CHOPPY")
        ptier   = s.get("portfolio_tier_v10", "B")
        rw      = REGIME_RANK_WEIGHT.get(regime, 0.5)
        tb      = TIER_BONUS.get(ptier, 1.0)
        pair    = s.get("pair", "")

        # Use live EV if available
        live_ev = calc_expected_value_r_live(conf, regime=regime, pair=pair)
        composite = conf * max(live_ev, ev_r) * rw * tb
        scored.append((composite, s))

    # Sort descending, return top N
    scored.sort(key=lambda x: x[0], reverse=True)
    ranked = [s for _, s in scored[:max_slots]]
    log(f"   📊 [RANK] {len(signals)} signals → ranked top {len(ranked)} "
        f"(max_slots={max_slots})")
    return ranked


# ════════════════════════════════════════════════════════
#  V10: EQUITY-BASED POSITION SCALING  [FIX #9]
# ════════════════════════════════════════════════════════
# Position size adapts to DD and equity growth automatically.

def calc_equity_scaling_mult() -> float:
    """
    [v10.1 FIX #9] Dynamic size multiplier based on portfolio performance.

    If equity is growing:  scale up gradually (max 1.3×)
    If drawdown active:    scale down aggressively (min 0.5×)
    Normal conditions:     1.0×

    This is applied ON TOP of the tier and volatility scalar.
    """
    try:
        state   = _load_paper_portfolio()
        cur_eq  = state.get("total_equity", INITIAL_EQUITY_USDT)
        peak_eq = state.get("peak_equity",  INITIAL_EQUITY_USDT)
        init_eq = INITIAL_EQUITY_USDT

        if peak_eq <= 0:
            return 1.0

        dd_pct = (peak_eq - cur_eq) / peak_eq

        # Drawdown scaling — reduce size as DD grows
        if dd_pct >= 0.15:  return 0.50   # -15%+ DD → half size
        if dd_pct >= 0.10:  return 0.65   # -10% DD
        if dd_pct >= 0.07:  return 0.80   # -7% DD
        if dd_pct >= 0.05:  return 0.90   # -5% DD

        # Growth scaling — reward profitable period
        growth = (cur_eq - init_eq) / init_eq
        if growth >= 0.30:  return 1.30   # +30% equity growth
...
# ════════════════════════════════════════════════════════
#  V10: PAPER TRADE JOURNAL ENGINE  [FIX #10]
# ════════════════════════════════════════════════════════
# Every closed trade records full context for edge analysis.

def save_trade_journal_entry(sig: dict, result: str, pnl_usdt: float,
                              exit_price: float, mae_usdt: float = 0.0,
                              mfe_usdt: float = 0.0,
                              hold_minutes: int = 0) -> None:
    """
    [v10.1 FIX #10] Save rich trade journal entry on close.

    Stores: regime, ATR, confidence, BTC context, volatility,
            spread, slippage, MAE, MFE, hold_time, setup.
    Enables edge analysis: which setups are most profitable.
    """
    try:
        entry_price = sig.get("entry", 0.0)
        size_usdt   = sig.get("position_size", 0.0)
        entry_val   = float(entry_price) if entry_price else 0.0
        atr_pct     = 0.0
        if entry_val > 0:
            # Reconstruct ATR% from SL distance as proxy
            sl_val = float(sig.get("sl", entry_val))
            atr_pct = abs(entry_val - sl_val) / entry_val * 100

        # R calculation
        sl_dist = abs(entry_val - float(sig.get("sl", entry_val)))
        risk_usdt = size_usdt * sl_dist / entry_val if entry_val > 0 and sl_dist > 0 else size_usdt * 0.007
        r_val   = pnl_usdt / risk_usdt if risk_usdt > 0 else 0
        ...
        payload = {
            ...
            "side":             sig.get("side", "BUY"),
            "result":           result,
            "closed_at":        datetime.now(WIB).isoformat(),
            # Execution
            "entry_price":      round(entry_val, 8),
            "exit_price":       round(exit_price, 8),
            "pnl_usdt":         round(pnl_usdt, 4),
            "pnl_r":            round(r_val, 3),
            "position_size":    round(size_usdt, 2),
            "hold_minutes":     hold_minutes,
            # Context at entry
            "regime":           sig.get("regime", "?"),
            "atr_pct":          round(atr_pct, 4),
            "confidence":       round(sig.get("confidence_v10", 0.0), 3),
            "score":            round(sig.get("score", 0.0), 3),
            "expected_value_r": round(sig.get("expected_value_r", 0.0), 3),
            "risk_grade":       sig.get("risk_grade_v10", "?"),
            "portfolio_tier":   sig.get("portfolio_tier_v10", "?"),
            "btc_trend":        "BEARISH" if sig.get("btc_bearish_trend") else "BULLISH",
            "rsi_at_entry":     round(sig.get("rsi", 50.0), 1),
            "rr_ratio":         round(sig.get("rr", 0.0), 2),
            # Risk metrics
            "mae_usdt":         round(mae_usdt, 4),
            "mfe_usdt":         round(mfe_usdt, 4),
            "sl_price":         round(float(sig.get("sl", 0)), 8),
            "tp1_price":        round(float(sig.get("tp1", 0)), 8),
            "tp2_price":        round(float(sig.get("tp2") or 0), 8),
            # Setup quality
            "setup_reason":     sig.get("reason", ""),
            "is_breakout":      bool(sig.get("structure", {}).get("bos")),
        }
        supabase.table("trade_journal").insert(payload).execute()
        log(f"   📔 [JOURNAL] {sig.get('pair')} {result} "
            f"R={r_val:+.2f} MAE=${mae_usdt:.2f} MFE=${mfe_usdt:.2f}")
    except Exception as e:
        log(f"   ⚠️ save_trade_journal_entry failed (non-critical): {e}", "warn")

def get_edge_analysis_summary() -> str:
    """
    [v10.1 FIX #10] Query trade_journal and return edge analysis summary.
    Shows: best regime, best setup, best session performance.
    """
    try:
        rows = (
            supabase.table("trade_journal")
            .select("regime, portfolio_tier, pnl_r, result, is_breakout")
        ...
        if not rows:
            return "📔 Trade journal kosong — belum ada data edge."
        ...
        for r in rows:
            ...
            if reg not in regime_stats:
                regime_stats[reg] = []
            regime_stats[reg].append(pnl_r)

        regime_ev = {k: (sum(v)/len(v), len(v)) for k, v in regime_stats.items()}
        best_reg  = max(regime_ev, key=lambda k: regime_ev[k][0]) if regime_ev else "—"

        # Tier performance
        tier_stats: dict = {}
        for r in rows:
            tier = r.get("portfolio_tier", "?")
            pnl_r = float(r.get("pnl_r") or 0)
            if tier not in tier_stats:
                tier_stats[tier] = []
            tier_stats[tier].append(pnl_r)

        tier_ev = {k: (sum(v)/len(v), len(v)) for k, v in tier_stats.items()}

        # Breakout vs continuation
        bos_trades  = [float(r.get("pnl_r") or 0) for r in rows if r.get("is_breakout")]
        cont_trades = [float(r.get("pnl_r") or 0) for r in rows if not r.get("is_breakout")]
        avg_bos  = sum(bos_trades) / len(bos_trades)   if bos_trades  else 0
        avg_cont = sum(cont_trades) / len(cont_trades)  if cont_trades else 0

        lines = ["📔 <b>EDGE ANALYSIS</b>\n━━━━━━━━━━━━━━━━━━━━━"]
        lines.append(f"Best Regime : <b>{best_reg}</b> (avg EV {regime_ev.get(best_reg, (0,0))[0]:+.2f}R)")
        lines.append("\nPerformance per Regime:")
        for reg, (avg_ev, n) in sorted(regime_ev.items(), key=lambda x: x[1][0], reverse=True):
            lines.append(f"  {reg}: {avg_ev:+.2f}R  (n={n})")
        lines.append("\nPerformance per Tier:")
        for tier, (avg_ev, n) in sorted(tier_ev.items(), key=lambda x: x[1][0], reverse=True):
            lines.append(f"  Tier {tier}: {avg_ev:+.2f}R  (n={n})")
        lines.append(f"\nBreakout vs Continuation:")
        lines.append(f"  BOS/CHoCH  : avg {avg_bos:+.2f}R  (n={len(bos_trades)})")
        lines.append(f"  Continuation: avg {avg_cont:+.2f}R  (n={len(cont_trades)})")
        return "\n".join(lines)

    except Exception as e:
        return f"⚠️ Edge analysis error: {e}"


# ════════════════════════════════════════════════════════
#  V10.2: LIVE EXPECTANCY TRACKER  [PRIORITY #1]
# ════════════════════════════════════════════════════════
# Tracks EV per: regime, confidence bucket, session,
# pair, weekday — the edge is always in a subset, not all market.

# Confidence buckets: 50-60%, 60-70%, 70-80%, 80-90%, 90%+
_CONF_BUCKETS = [(0.90, "90%+"), (0.80, "80-90%"), (0.70, "70-80%"),
                 (0.60, "60-70%"), (0.50, "50-60%"), (0.0,  "<50%")]

def _conf_bucket(conf: float) -> str:
    for threshold, label in _CONF_BUCKETS:
        if conf >= threshold:
            return label
    return "<50%"
...
_SESSIONS = {
    "ASIA":    (1,  8),    # 08:00–15:00 WIB
    "EUROPE":  (7,  16),   # 14:00–23:00 WIB
    "US":      (13, 21),   # 20:00–04:00 WIB
    "LATE_US": (21, 24),
}
...
_expectancy_tracker_cache: dict = {"ts": 0.0, "data": None}
EXPECTANCY_TRACKER_TTL = 3600  # refresh every 1 hour

def build_expectancy_tracker(limit: int = 500) -> dict:
    """
    [v10.2 #1] Query trade_journal and segment EV by:
      - regime, confidence_bucket, session, pair, weekday

    Returns dict[dimension][segment] = {n, avg_r, win_rate, ev_r}
    Cached for EXPECTANCY_TRACKER_TTL seconds.
    """
    global _expectancy_tracker_cache
    now = time.time()
    if (_expectancy_tracker_cache["data"] is not None and
            now - _expectancy_tracker_cache["ts"] < EXPECTANCY_TRACKER_TTL):
        return _expectancy_tracker_cache["data"]

    result: dict = {
        "regime": {}, "conf_bucket": {}, "session": {},
        "pair": {}, "weekday": {}, "overall": {},
    }
    try:
        rows = (
            supabase.table("trade_journal")
            .select("pair, result, pnl_r, confidence, regime, closed_at, rsi_at_entry")
            .not_.is_("pnl_r", "null")
            .order("closed_at", desc=True)
            .limit(limit)
            .execute()
        ...
        if not rows:
            _expectancy_tracker_cache = {"ts": now, "data": result}
            return result

        def _add_to(dim: dict, key: str, r_val: float, is_win: bool):
            if key not in dim:
                dim[key] = {"n": 0, "wins": 0, "r_sum": 0.0, "r_vals": []}
            dim[key]["n"]      += 1
            dim[key]["wins"]   += int(is_win)
            dim[key]["r_sum"]  += r_val
            dim[key]["r_vals"].append(r_val)

        for row in rows:
            pnl_r  = float(row.get("pnl_r") or 0)
            conf   = float(row.get("confidence") or 0.5)
            regime = row.get("regime") or "UNKNOWN"
            pair   = (row.get("pair") or "?").replace("_USDT", "")
            closed = row.get("closed_at") or ""
            result_str = (row.get("result") or "").upper()
            is_win = result_str in ("WIN", "TP1", "TP2", "PARTIAL_WIN")

            # Weekday + session from closed_at
            try:
                dt_closed  = datetime.fromisoformat(closed.replace("Z", "+00:00"))
                wday_name  = _WEEKDAY_NAMES[dt_closed.weekday()]
                sess       = _detect_session(dt_closed.hour)
            except Exception:
                wday_name, sess = "?", "?"

            _add_to(result["regime"],      regime,              pnl_r, is_win)
            _add_to(result["conf_bucket"], _conf_bucket(conf),  pnl_r, is_win)
            _add_to(result["session"],     sess,                pnl_r, is_win)
            _add_to(result["pair"],        pair,                pnl_r, is_win)
            _add_to(result["weekday"],     wday_name,           pnl_r, is_win)
            _add_to(result["overall"],     "ALL",               pnl_r, is_win)

        # Compute summary stats per segment
        for dim_data in result.values():
            for seg, stats in dim_data.items():
                n = stats["n"]
                if n == 0:
                    continue
                avg_r  = stats["r_sum"] / n
                wr     = stats["wins"] / n * 100
                r_vals = stats["r_vals"]
                # Sharpe (simplified, no annualisation)
                if len(r_vals) > 1:
                    import statistics as _st
                    mu  = _st.mean(r_vals)
                    std = _st.stdev(r_vals)
        ...
        _expectancy_tracker_cache = {"ts": now, "data": result}
        log(f"   📊 [EV_TRACKER] {len(rows)} trades segmented across 5 dimensions")
        return result

    except Exception as e:
        log(f"⚠️ build_expectancy_tracker: {e}", "warn")
        return result


def format_ev_report(tracker: dict) -> str:
    """Format expectancy tracker into Telegram-ready report."""
    lines = ["📊 <b>LIVE EXPECTANCY REPORT v10.2</b>\n━━━━━━━━━━━━━━━━━━━━━"]
    overall = tracker.get("overall", {}).get("ALL", {})
    if overall.get("n", 0) > 0:
        lines.append(
            f"Overall : n={overall['n']}  AvgR={overall['avg_r']:+.3f}  "
            f"WR={overall['win_rate']:.1f}%  Sharpe={overall['sharpe']:.2f}"
        )

    for dim_label, dim_key in [
        ("By Regime",     "regime"),
        ("By Confidence", "conf_bucket"),
        ("By Session",    "session"),
        ("By Weekday",    "weekday"),
    ]:
        dim_data = tracker.get(dim_key, {})
        if not dim_data:
            continue
        lines.append(f"\n<b>{dim_label}:</b>")
        sorted_segs = sorted(dim_data.items(),
                             key=lambda x: x[1].get("avg_r", 0), reverse=True)
        for seg, st in sorted_segs:
            if st.get("n", 0) < 3:   # skip tiny samples
                continue
            edge_marker = "✅" if st["avg_r"] > 0.1 else ("⚠️" if st["avg_r"] > 0 else "❌")
            lines.append(
                f"  {edge_marker} {seg}: n={st['n']}  "
                f"AvgR={st['avg_r']:+.3f}  WR={st['win_rate']:.1f}%"
            )

    # Top 5 pairs
    pair_data = tracker.get("pair", {})
    if pair_data:
        lines.append("\n<b>Top Pairs (by AvgR):</b>")
        top5 = sorted(pair_data.items(),
                      key=lambda x: x[1].get("avg_r", -99), reverse=True)[:5]
        for pair, st in top5:
            if st.get("n", 0) < 2:
                continue
            lines.append(
                f"  {pair}: n={st['n']} AvgR={st['avg_r']:+.3f} "
                f"WR={st['win_rate']:.1f}%"
            )
...
def send_ev_report() -> None:
    """Send expectancy tracker report to Telegram."""
    tracker = build_expectancy_tracker()
    tg_async(format_ev_report(tracker))
    log("📊 [EV_TRACKER] Expectancy report dikirim.")


def get_segment_ev(regime: str = "", conf: float = 0.0,
                   pair: str = "", session: str = "") -> float:
    """
    [v10.2] Return best known EV for this signal's specific context.
    Used by rank_signals() for more accurate ranking.
    Falls back to overall EV if segment not found.
    """
    tracker = build_expectancy_tracker()
    # Try most specific → least specific
    candidates = []
    if regime:
        seg = tracker.get("regime", {}).get(regime, {})
        if seg.get("n", 0) >= 5:
            candidates.append(seg["avg_r"])
    if conf:
        bucket = _conf_bucket(conf)
        seg = tracker.get("conf_bucket", {}).get(bucket, {})
        if seg.get("n", 0) >= 5:
            candidates.append(seg["avg_r"])
    if pair:
        p = pair.replace("_USDT", "")
        seg = tracker.get("pair", {}).get(p, {})
        if seg.get("n", 0) >= 3:
            candidates.append(seg["avg_r"])
    if session:
        seg = tracker.get("session", {}).get(session, {})
        if seg.get("n", 0) >= 5:
            candidates.append(seg["avg_r"])

    if candidates:
        return sum(candidates) / len(candidates)   # weighted mean of available segments

    # Fallback: overall
    overall = tracker.get("overall", {}).get("ALL", {})
    return overall.get("avg_r", 0.0)


# ════════════════════════════════════════════════════════
#  V10.2: SIGNAL QUALITY ANALYTICS  [PRIORITY #2]
# ════════════════════════════════════════════════════════
# MAE histogram, MFE histogram, TP1→TP2 conversion rate,
# RR distribution, expectancy by confidence bucket.

def build_signal_quality_analytics(limit: int = 200) -> dict:
    """
    [v10.2 #2] Analyse closed trades for quality insights.

    Returns dict with:
      mae_hist      — MAE distribution (how much drawdown before outcome)
      mfe_hist      — MFE distribution (max unrealized profit before outcome)
      tp1_to_tp2    — of trades that hit TP1, how many continued to TP2
      rr_dist       — actual RR distribution vs planned
      ev_by_conf    — expectancy per confidence bucket
      avg_hold_mins — average hold time per result type
    """
    try:
        rows = (
            supabase.table("trade_journal")
            .select("result, pnl_r, mae_usdt, mfe_usdt, confidence, "
                    "rr_ratio, hold_minutes, entry_price, position_size")
            .not_.is_("pnl_r", "null")
            .order("closed_at", desc=True)
            .limit(limit)
            .execute()
        ...
        tp1_count, tp2_count = 0, 0
        hold_by_result: dict = {}
        ev_by_conf: dict = {}

        for r in rows:
            result_str = (r.get("result") or "").upper()
            pnl_r      = float(r.get("pnl_r") or 0)
            mae        = float(r.get("mae_usdt") or 0)
            mfe        = float(r.get("mfe_usdt") or 0)
            conf       = float(r.get("confidence") or 0.5)
            rr         = float(r.get("rr_ratio") or 0)
            hold_m     = int(r.get("hold_minutes") or 0)

            if mae > 0:   mae_vals.append(mae)
            if mfe > 0:   mfe_vals.append(mfe)
            if rr > 0:    rr_vals.append(rr)

            # TP1 → TP2 conversion
            if result_str in ("TP1", "PARTIAL_WIN"):
                tp1_count += 1
            if result_str in ("TP2", "WIN"):
                tp1_count += 1
                tp2_count += 1

            # Hold time
            if result_str not in hold_by_result:
                hold_by_result[result_str] = []
            if hold_m > 0:
                hold_by_result[result_str].append(hold_m)

            # EV by confidence bucket
            bucket = _conf_bucket(conf)
            if bucket not in ev_by_conf:
                ev_by_conf[bucket] = {"vals": [], "wins": 0}
            ev_by_conf[bucket]["vals"].append(pnl_r)
            is_win = result_str in ("WIN", "TP1", "TP2", "PARTIAL_WIN")
            ev_by_conf[bucket]["wins"] += int(is_win)

        def _hist(vals, bins=5):
            """Simple histogram as list of (label, count)."""
            if not vals:
                return []
            mn, mx = min(vals), max(vals)
            if mn == mx:
                return [(f"{mn:.2f}", len(vals))]
            step = (mx - mn) / bins
            buckets = []
            for i in range(bins):
                lo = mn + i * step
                ...
                buckets.append((f"{lo:.2f}–{hi:.2f}", cnt))
            return buckets

        # Compute EV summary per confidence bucket
        ev_summary: dict = {}
        for bucket, data in ev_by_conf.items():
            n = len(data["vals"])
            if n == 0:
                continue
            avg_r = sum(data["vals"]) / n
            wr    = data["wins"] / n * 100
            ev_summary[bucket] = {"n": n, "avg_r": round(avg_r, 3), "wr": round(wr, 1)}

        # Average hold time
        avg_hold = {
            res: round(sum(v) / len(v)) if v else 0
            for res, v in hold_by_result.items()
        }

        return {
            "status":         "ok",
            "n_trades":       len(rows),
            "mae_hist":       _hist(mae_vals),
            "mfe_hist":       _hist(mfe_vals),
            "tp1_count":      tp1_count,
            "tp2_count":      tp2_count,
            "tp1_to_tp2_pct": round(tp2_count / tp1_count * 100, 1) if tp1_count > 0 else 0,
            "rr_avg":         round(sum(rr_vals) / len(rr_vals), 2) if rr_vals else 0,
            "rr_median":      round(sorted(rr_vals)[len(rr_vals)//2], 2) if rr_vals else 0,
            "ev_by_conf":     ev_summary,
            "avg_hold_mins":  avg_hold,
        }
    except Exception as e:
        log(f"⚠️ build_signal_quality_analytics: {e}", "warn")
        return {"status": "error", "msg": str(e)}


def format_signal_quality_report(qa: dict) -> str:
    """Format signal quality analytics into Telegram message."""
    if qa.get("status") != "ok":
        return f"📊 Signal Quality: {qa.get('status','?')} {qa.get('msg','')}"

    lines = [
        f"🔬 <b>SIGNAL QUALITY ANALYTICS v10.2</b>  (n={qa['n_trades']})\n"
        f"━━━━━━━━━━━━━━━━━━━━━",
        f"TP1 → TP2 Conversion : <b>{qa['tp1_to_tp2_pct']:.1f}%</b>  "
        f"({qa['tp2_count']}/{qa['tp1_count']} trades)",
        f"Avg R/R (actual)     : <b>{qa['rr_avg']:.2f}</b>  "
        f"(median {qa['rr_median']:.2f})\n",
    ]

    lines.append("<b>EV by Confidence Bucket:</b>")
    for bucket in ["90%+", "80-90%", "70-80%", "60-70%", "50-60%", "<50%"]:
        d = qa["ev_by_conf"].get(bucket)
        if not d or d["n"] < 2:
            continue
        edge = "✅" if d["avg_r"] > 0.05 else ("⚠️" if d["avg_r"] >= 0 else "❌")
        lines.append(
            f"  {edge} {bucket}: n={d['n']}  "
            f"AvgR={d['avg_r']:+.3f}  WR={d['wr']:.1f}%"
        )

    lines.append("\n<b>MAE Distribution (USDT):</b>")
    for label, cnt in qa.get("mae_hist", []):
        bar = "█" * min(cnt, 10)
        lines.append(f"  {label}: {bar} ({cnt})")

    lines.append("\n<b>MFE Distribution (USDT):</b>")
    for label, cnt in qa.get("mfe_hist", []):
        bar = "█" * min(cnt, 10)
        lines.append(f"  {label}: {bar} ({cnt})")

    lines.append("\n<b>Avg Hold Time per Result:</b>")
    for res, mins in sorted(qa.get("avg_hold_mins", {}).items()):
        h, m = divmod(mins, 60)
        lines.append(f"  {res}: {h}h {m}m")

    return "\n".join(lines)


def send_signal_quality_report() -> None:
    """Send signal quality analytics to Telegram."""
    qa  = build_signal_quality_analytics()
    tg_async(format_signal_quality_report(qa))
    log("📊 [QA] Signal quality report dikirim.")


# ════════════════════════════════════════════════════════
#  V10.2: REGIME-ADAPTIVE STRATEGY FILTER  [PRIORITY #3]
# ════════════════════════════════════════════════════════
# Each regime → different signal type preference + size modifier.
# STRONG_TREND  → breakout only, full size
# EXPANSION     → breakout + trend continuation, full size
# COMPRESSION   → reduce size, mean reversion ok
# CHOPPY        → half size, only strongest setups
# PANIC         → block all BUY
# DISTRIBUTION  → prefer SELL, reduce BUY size

# Regime → {preferred_setups, size_mult, min_score, allow_buy, allow_sell, note}
REGIME_STRATEGY_MAP = {
    "STRONG_TREND": {
        "preferred_setups":  ["breakout", "momentum"],
        "size_mult":         1.0,
        "min_score":         3.5,
        "allow_buy":         True,
        "allow_sell":        False,
        "note":              "Momentum breakout. Full size. Trend is friend.",
    },
    "EXPANSION": {
        "preferred_setups":  ["breakout", "continuation"],
        "size_mult":         1.0,
        "min_score":         3.5,
        "allow_buy":         True,
        "allow_sell":        False,
        "note":              "Expansion phase. Full size, allow all breakouts.",
    },
    "TRENDING": {   # Legacy ADX-based label
        "preferred_setups":  ["breakout", "continuation", "momentum"],
        "size_mult":         1.0,
        "min_score":         3.5,
        "allow_buy":         True,
        "allow_sell":        True,
        "note":              "ADX Trending. Normal operation.",
    },
    "COMPRESSION": {
        "preferred_setups":  ["mean_reversion", "range_bound"],
        "size_mult":         0.6,
        "min_score":         4.0,   # Needs stronger signal
        "allow_buy":         True,
        "allow_sell":        True,
        "note":              "Range bound. Reduce size. Mean reversion setups ok.",
    },
    "CHOPPY": {
        "preferred_setups":  [],    # Only take very high conviction
        "size_mult":         0.5,
        "min_score":         4.2,   # Very high threshold
        "allow_buy":         True,
        "allow_sell":        True,
        "note":              "Choppy. Half size. Only 90%+ confidence signals.",
    },
    "RANGING": {   # Legacy ADX-based label
        "preferred_setups":  ["mean_reversion"],
        "size_mult":         0.7,
        "min_score":         4.0,
        "allow_buy":         True,
        "allow_sell":        True,
        "note":              "Ranging. Reduce size. Stricter threshold.",
    },
    "DISTRIBUTION": {
        "preferred_setups":  ["sell_setup", "breakdown"],
        "size_mult":         0.4,
        "min_score":         4.0,
        "allow_buy":         False,  # Avoid new longs
        "allow_sell":        True,
        "note":              "Distribution. Avoid BUY. SELL only. Small size.",
    },
    "PANIC": {
        "preferred_setups":  [],
        "size_mult":         0.0,
        "min_score":         9.9,   # Effectively blocks everything
        "allow_buy":         False,
        "allow_sell":        False,
        "note":              "PANIC. ALL entries blocked.",
    },
}

def apply_regime_adaptive_filter(sig: dict, btc_regime: str) -> tuple[bool, float]:
    """
    [v10.2 #3] Apply regime-specific filtering to a signal.

    Returns: (passes: bool, size_multiplier: float)
    - passes=False means this signal should be skipped entirely
    - size_multiplier overrides regime size for this specific signal

    Logic:
      1. Check if side (BUY/SELL) is allowed in current regime
      2. Check if score meets regime's elevated minimum
      3. Return appropriate size multiplier
    """
    regime_cfg = REGIME_STRATEGY_MAP.get(btc_regime, REGIME_STRATEGY_MAP["RANGING"])
    side       = sig.get("side", "BUY")
    score      = sig.get("score", 0.0)
    confidence = sig.get("confidence_v10", 0.0)

    # Check side permission
    if side == "BUY"  and not regime_cfg["allow_buy"]:
        log(f"   🚫 [REGIME_FILTER] {sig.get('pair')} BUY blocked — "
            f"regime {btc_regime} ({regime_cfg['note']})")
        return False, 0.0

    if side == "SELL" and not regime_cfg["allow_sell"]:
        log(f"   🚫 [REGIME_FILTER] {sig.get('pair')} SELL blocked — "
            f"regime {btc_regime}")
        return False, 0.0

    # Check minimum score for this regime
    min_score = regime_cfg["min_score"]
    if score < min_score:
        log(f"   🚫 [REGIME_FILTER] {sig.get('pair')} score {score:.2f} < "
            f"{min_score:.1f} (regime={btc_regime} threshold)")
        return False, 0.0

    # PANIC hard block
    if regime_cfg["size_mult"] == 0.0:
        return False, 0.0

    return True, regime_cfg["size_mult"]


# ════════════════════════════════════════════════════════
#  V10.2: CORRELATION-AWARE PORTFOLIO OPTIMIZER  [PRIORITY #4]
# ════════════════════════════════════════════════════════
# When multiple signals from the same sector/cluster exist,
# pick only the top-ranked one instead of taking all.
# Avoids loading 4 AI coins simultaneously.

def optimize_portfolio_correlation(signals: list, tracker: dict | None = None) -> list:
    """
    [v10.2 #4] Correlation-aware portfolio optimizer.

    Rules:
      1. Group signals by sector cluster (AI, MEME, L2, BTC)
      2. Within each cluster, keep only the top-1 signal by composite score
      3. Unclassified pairs: take all (up to slot limit)
      4. Integrate live EV from tracker for better ranking

    This ensures:
      - No more than 1 AI coin, 1 MEME, 1 L2 at a time
      - Overall portfolio beta stays diversified
      - Best signal in each cluster wins the slot
    ...
    def _composite(s: dict) -> float:
        ...
        regime  = s.get("regime", "CHOPPY")
        pair    = s.get("pair", "")
        session_ev = get_segment_ev(regime=regime, conf=conf, pair=pair)
        rw      = REGIME_RANK_WEIGHT.get(regime, 0.5)
        return conf * max(session_ev, s.get("expected_value_r", 0.0)) * rw + score * 0.1

    # Group by cluster
    cluster_best: dict[str, tuple] = {}   # cluster → (composite, signal)
    unclassified: list = []

    for s in signals:
        pair    = s.get("pair", "")
        cluster = get_pair_cluster(pair)   # existing function in bot
        if cluster is None or cluster == "":
            unclassified.append((_composite(s), s))
        else:
            comp = _composite(s)
            if cluster not in cluster_best or comp > cluster_best[cluster][0]:
                cluster_best[cluster] = (comp, s)

    # Build optimized list: 1 best per cluster + all unclassified
    optimized = [s for _, s in cluster_best.values()]
    optimized += [s for _, s in sorted(unclassified, key=lambda x: x[0], reverse=True)]

    # Sort final list by composite
    optimized.sort(key=_composite, reverse=True)

    skipped = len(signals) - len(optimized)
    if skipped > 0:
        log(f"   🎯 [PORTFOLIO_OPT] {len(signals)} signals → "
            f"{len(optimized)} after correlation filter "
            f"({skipped} deduped from same cluster)")

    return optimized


# ════════════════════════════════════════════════════════
#  V10.2: FORWARD TEST HARNESS  [PRIORITY #5]
# ════════════════════════════════════════════════════════
# Run statistical analysis on trade_journal + signals_v2
# for Sharpe, Sortino, PF, MAE/MFE, expectancy validation.
# Use after 50+ trades for meaningful results.

@dataclass
class ForwardTestResult:
    """Summary statistics from forward test analysis."""
    n_trades:       int
    n_wins:         int
    n_losses:       int
    win_rate:       float
    ...
    avg_mae:        float
    avg_mfe:        float
    tp1_to_tp2_pct: float
    best_regime:    str
    worst_regime:   str
    best_conf_bucket: str

def run_forward_test_analysis(limit: int = 500) -> ForwardTestResult | None:
    """
    [v10.2 #5] Analyse full trade history for statistical edge validation.

    Fetches from trade_journal (rich context) + signals_v2 (results).
    Minimum 10 trades to produce meaningful output.

    Returns ForwardTestResult dataclass or None if insufficient data.
    """
    try:
        rows = (
            supabase.table("trade_journal")
            .select("result, pnl_r, mae_usdt, mfe_usdt, confidence, "
                    "regime, entry_price, position_size, rr_ratio")
            .not_.is_("pnl_r", "null")
            .order("closed_at", desc=True)
            .limit(limit)
            .execute()
        ...
        if len(rows) < 10:
            log(f"⚠️ [FWD_TEST] Hanya {len(rows)} trades — butuh minimal 10 untuk analisis")
            return None

        WIN_RESULTS  = {"WIN", "TP1", "TP2", "PARTIAL_WIN"}
        LOSS_RESULTS = {"LOSS", "SL", "EXPIRED_LOSS", "SL_AFTER_TP1"}
        ...
        mae_vals: list[float] = []
        mfe_vals: list[float] = []
        regime_r: dict[str, list] = {}
        conf_r:   dict[str, list] = {}

        for row in rows:
            res   = (row.get("result") or "").upper()
            pnl_r = float(row.get("pnl_r") or 0)
            mae   = float(row.get("mae_usdt") or 0)
            mfe   = float(row.get("mfe_usdt") or 0)
            conf  = float(row.get("confidence") or 0.5)
            reg   = row.get("regime") or "UNKNOWN"

            if res not in WIN_RESULTS and res not in LOSS_RESULTS:
                continue   # skip BREAKEVEN, EXPIRED_PROFIT, etc.

            r_vals.append(pnl_r)
            if res in WIN_RESULTS:  win_rs.append(pnl_r)
            else:                   loss_rs.append(abs(pnl_r))
            if mae > 0:   mae_vals.append(mae)
            if mfe > 0:   mfe_vals.append(mfe)

            if reg not in regime_r:  regime_r[reg] = []
            regime_r[reg].append(pnl_r)

            bucket = _conf_bucket(conf)
            if bucket not in conf_r:  conf_r[bucket] = []
            conf_r[bucket].append(pnl_r)
        ...
        wr          = n_wins / n
        avg_win     = sum(win_rs)  / n_wins  if win_rs  else 0
        avg_loss    = sum(loss_rs) / n_loss  if loss_rs else 0
        ...
        import statistics as _st
        mu  = _st.mean(r_vals)
        std = _st.stdev(r_vals) if len(r_vals) > 1 else 0
        sharpe = mu / std if std > 0 else 0

        neg_vals = [r for r in r_vals if r < 0]
        downstd  = _st.stdev(neg_vals) if len(neg_vals) > 1 else 0
        sortino  = mu / downstd if downstd > 0 else 0

        # Max DD in R terms (sequential drawdown)
        peak_r  = 0.0;  max_dd_r = 0.0
        cum_r   = 0.0
        for r in r_vals:
            cum_r += r
        ...
        for row in rows:
            res = (row.get("result") or "").upper()
            if res in LOSS_RESULTS:
                streak += 1
                max_streak = max(max_streak, streak)
            elif res in WIN_RESULTS:
                streak = 0
        ...
        # Best/worst regime
        regime_avg = {k: sum(v)/len(v) for k, v in regime_r.items() if len(v) >= 3}
        best_regime  = max(regime_avg, key=regime_avg.get) if regime_avg else "—"
        worst_regime = min(regime_avg, key=regime_avg.get) if regime_avg else "—"

        # Best confidence bucket
        conf_avg = {k: sum(v)/len(v) for k, v in conf_r.items() if len(v) >= 3}
        best_conf = max(conf_avg, key=conf_avg.get) if conf_avg else "—"

        # TP1→TP2 (from qa cache)
        qa = build_signal_quality_analytics(limit=limit)
        tp_pct = qa.get("tp1_to_tp2_pct", 0.0)
        ...
        return ForwardTestResult(
            ...
            tp1_to_tp2_pct = tp_pct,
            best_regime    = best_regime,
            worst_regime   = worst_regime,
            best_conf_bucket = best_conf,
        )

    except Exception as e:
        log(f"⚠️ run_forward_test_analysis: {e}", "error")
        return None


def format_forward_test_report(r: ForwardTestResult) -> str:
    """Format ForwardTestResult into Telegram message."""
    wr_icon = "🟢" if r.win_rate >= 55 else ("🟡" if r.win_rate >= 45 else "🔴")
    ev_icon = "✅" if r.expectancy_r > 0.05 else ("⚠️" if r.expectancy_r >= 0 else "❌")
    pf_icon = "✅" if r.profit_factor > 1.2 else ("⚠️" if r.profit_factor > 1.0 else "❌")

    return (
        f"🧪 <b>FORWARD TEST ANALYSIS v10.2</b>  (n={r.n_trades})\n"
        f"━━━━━━━━━━━━━━━━━━━━━\n"
        f"Win Rate      : {wr_icon} <b>{r.win_rate:.1f}%</b>  "
        f"({r.n_wins}W / {r.n_losses}L)\n"
        f"Expectancy    : {ev_icon} <b>{r.expectancy_r:+.3f}R</b> per trade\n"
        f"Profit Factor : {pf_icon} <b>{r.profit_factor:.2f}</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━━\n"
        f"Avg Win R     : +{r.avg_win_r:.3f}R\n"
        f"Avg Loss R    : -{r.avg_loss_r:.3f}R\n"
        f"Sharpe        : <b>{r.sharpe:.2f}</b>\n"
        f"Sortino       : <b>{r.sortino:.2f}</b>\n"
        f"Max DD (R)    : <b>{r.max_dd_r:.2f}R</b>  "
        f"(streak {r.max_dd_streak})\n"
        f"━━━━━━━━━━━━━━━━━━━━━\n"
        f"TP1→TP2 conv  : <b>{r.tp1_to_tp2_pct:.1f}%</b>\n"
        f"Avg MAE       : ${r.avg_mae:.2f}  "
        f"Avg MFE: ${r.avg_mfe:.2f}\n"
        f"━━━━━━━━━━━━━━━━━━━━━\n"
        f"Best Regime   : <b>{r.best_regime}</b>\n"
        f"Worst Regime  : <b>{r.worst_regime}</b>\n"
        f"Best Conf     : <b>{r.best_conf_bucket}</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━━\n"
        f"<i>Minimum 300–500 trades untuk Sharpe yang reliable.</i>"
    )


def send_forward_test_report() -> None:
    """Run forward test analysis and send to Telegram."""
    result = run_forward_test_analysis()
    if result is None:
        tg_async("🧪 Forward Test: belum cukup data (min 10 trades).")
        return
    tg_async(format_forward_test_report(result))
    log(f"🧪 [FWD_TEST] Report dikirim: "
        f"n={result.n_trades} WR={result.win_rate}% "
        f"EV={result.expectancy_r:+.3f}R PF={result.profit_factor:.2f}")
...
def run():
    ...
    # ── [V10.2] On-demand analytics modes ────────────────────────
    # SCAN_MODE=analytics → send full analytics suite without trading
    if SCAN_MODE == "analytics":
        log("📊 SCAN_MODE=analytics — kirim full analytics suite")
        tg(f"📊 <b>Manual Analytics Request</b>\n{datetime.now(WIB).strftime('%Y-%m-%d %H:%M WIB')}")
        tg_async(build_analytics_dashboard())
        send_ev_report()
        send_signal_quality_report()
        tg_async(get_edge_analysis_summary())
        tg_drain(timeout=30.0)
        log("✅ Analytics suite terkirim.")
        return

    # SCAN_MODE=forward_test → run full statistical forward test
    if SCAN_MODE == "forward_test":
        log("🧪 SCAN_MODE=forward_test — run statistical analysis")
        send_forward_test_report()
        tg_drain(timeout=30.0)
        log("✅ Forward test report terkirim.")
        return
    ...
    # ── [V10] DYNAMIC SLOT ENGINE ─────────────────────────────────
    # Reduce active slots based on DD + streak + BTC conditions.
    # This overrides MAX_OPEN_TRADES dynamically during the cycle.
    _dd_pct          = drawdown.get("dd_pct", 0.0)
    _streak          = drawdown.get("streak", 0)
    _v10_max_slots   = get_dynamic_max_slots(_dd_pct, _streak, btc)
    _v10_btc_regime  = get_v10_btc_regime_label(btc)
    _v10_open_trades = portfolio_state['total']
    _v10_slots_used  = _v10_open_trades
    _v10_slots_avail = max(0, _v10_max_slots - _v10_slots_used)
    log(f"⚡ [V10] Slots: {_v10_slots_used}/{_v10_max_slots} used "
        f"(DD={_dd_pct*100:.1f}% streak={_streak} regime={_v10_btc_regime}) "
        f"| Available: {_v10_slots_avail}")
    ...
    # ── [V10.2] Signal Pipeline ───────────────────────────────────
    # Step 1: Conflict resolution (existing logic)
    ...
    # Step 2: [V10.2 #3] Regime-Adaptive Filter
    # Each regime has its own min_score, allowed sides, preferred setups.
    _v10_btc_regime_now = get_v10_btc_regime_label(btc)
    _regime_filtered = []
    for _s in signals:
        _passes, _regime_size_mult = apply_regime_adaptive_filter(_s, _v10_btc_regime_now)
        if _passes:
            _s["_regime_size_mult"] = _regime_size_mult   # carry through to sizing
            _regime_filtered.append(_s)
    if len(_regime_filtered) < len(signals):
        log(f"   🎯 [REGIME_FILTER] {len(signals)} → {len(_regime_filtered)} "
            f"(regime={_v10_btc_regime_now})")
    signals = _regime_filtered

    # Step 3: [V10.2 #4] Correlation-Aware Portfolio Optimizer
    # Keep only the top signal per sector cluster.
    _ev_tracker = build_expectancy_tracker()
    signals = optimize_portfolio_correlation(signals, tracker=_ev_tracker)

    # Step 4: [V10.2 FIX #8] Rank by EV × Confidence × Regime Weight
    # Now uses live segment EV from tracker for more accurate ranking.
    signals = rank_signals(signals, max_slots=_v10_max_slots)
    ...
    for sig in signals:
        ...
        if portfolio_state["total"] >= _v10_max_slots:
            log(f"   🚫 Main signal hard-stop — "
                f"{portfolio_state['total']}/{_v10_max_slots} slots (dynamic)")
            break

        # [v7.11 #1] Portfolio Brain gate
        if not portfolio_allows(sig, portfolio_state, btc):
            continue

        # [v7.16 #A #D] Position sizing with regime multiplier
        ...
        # Apply regime-specific size multiplier
        _r_size_mult = sig.pop("_regime_size_mult", 1.0)
        if _r_size_mult != 1.0:
            sig["position_size"] = round(sig["position_size"] * _r_size_mult, 2)
            log(f"   📐 [REGIME_SIZE] {sig.get('pair')} "
                f"size × {_r_size_mult:.1f} = ${sig['position_size']:.2f}")
        ...
        # [v7.16 #B] Slippage-adjusted entry
        ...
        # ── [V10 FIX #5] Open paper position ─────────────────────
        try:
            update_paper_portfolio_open(sig, sig.get("position_size", BASE_POSITION_USDT))
        except Exception as _ppe:
            log(f"   ⚠️ paper portfolio open failed: {_ppe}", "warn")

        # ── [V10 FIX #7] Signal quality context ──────────────────
        try:
            _atr_pct_ctx = (float(sig.get("atr") or 0) / float(_entry or 1)) * 100 if _entry else 0
            save_signal_quality_context(
                sig, sig.get("confidence_v10", 0.0),
                sig.get("expected_value_r", 0.0),
                sig.get("risk_grade_v10", "B"),
                sig.get("position_size", BASE_POSITION_USDT),
                _atr_pct_ctx, btc,
            )
        except Exception as _sqe:
            log(f"   ⚠️ signal quality context failed: {_sqe}", "warn")
